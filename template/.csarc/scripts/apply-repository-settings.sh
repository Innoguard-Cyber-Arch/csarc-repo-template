...
ruleset_payload="$repo_root/.csarc/policies/rulesets.json"
release_policy="$repo_root/.csarc/policies/releases.json"
...
# out of .csarc/policies/repository.json's flat REST PATCH and declared here
# instead; it is only readable/writable through the GraphQL API.
issue_creation_policy_payload="$repo_root/.csarc/policies/issue-creation.json"
...
# PATCH atomically, so it is kept out of .csarc/policies/repository.json and PATCHed
# in its own dedicated call instead.
security_scanning_payload="$repo_root/.csarc/policies/security-scanning.json"
required_checks_ruleset_payload="$repo_root/.csarc/policies/rulesets-required-checks.json"
...
if [[ ! -f "$required_checks_ruleset_payload" ]]; then
  echo "Missing .csarc/policies/rulesets-required-checks.json." >&2
  exit 1
fi
...
config_reader="$repo_root/.csarc/scripts/csarc_config.py"
policy_config_value() {
...
print_ruleset_guidance() {
  ...
  echo "- PRESERVED desired Ruleset: .csarc/policies/rulesets.json keeps main branch governance ready for a supported plan or public repository."
  ...
  echo "  GH_REPO=$repo ./.csarc/scripts/apply-repository-settings.sh plan"
  echo "  GH_REPO=$repo ./.csarc/scripts/apply-repository-settings.sh apply"
}
...
if [[ "$mode" == "check" ]]; then
  ...
  if [[ "$policy_repository_settings" != "true" ]]; then
    echo "SKIPPED .csarc/policies/repository.json (policy_repository_settings=false in .csarc/config.yml)."
    ...
    if ! repository_drift="$(python3 - "$repo_root/.csarc/policies/repository.json" "$repository_state" 2>&1 <<'PY'
import json
import sys
...
PY
  ...
  if [[ "$repository_state_available" == "true" ]]; then
    repository_missing="$(python3 - "$repo_root/.csarc/policies/repository.json" "$repository_state" <<'PY'
import json
import sys
...
PY
    ...
    if [[ -n "$repository_missing" ]]; then
      ...
      echo "Repository settings match .csarc/policies/repository.json."
    fi
  fi
  ...
  if ! load_issue_creation_policy; then
    ...
    echo "Issue creation policy matches .csarc/policies/issue-creation.json."
  fi

  # security_and_analysis is compared per field, desired "enabled" but live
  # "disabled" (or missing) is treated as DEGRADED (a plan/GHAS limitation),
  # any other mismatch is treated as actionable drift. This mirrors the
  # "DEGRADED Actions PR policy" true-vs-false heuristic used for Actions
  # permissions. It is a distinct concern from the policy_repository_settings
  # toggle (Issue #532): that toggle only skips the .csarc/policies/repository.json
  # field-drift comparison below, so this fetches repository state on its
  # own rather than depending on the toggle-gated $repository_state there.
  if ! security_and_analysis_repository_state="$(gh api "repos/$repo" 2>&1)"; then
    echo "Cannot inspect security_and_analysis for $repo." >&2
    echo "$security_and_analysis_repository_state" >&2
    check_errors=$((check_errors + 1))
  elif ! security_and_analysis_status="$(python3 - "$security_scanning_payload" "$security_and_analysis_repository_state" 2>&1 <<'PY'
import json
import sys
...
PY
    ...
    while IFS='|' read -r security_field_kind security_field_name security_field_value; do
      ...
      case "$security_field_kind" in
        MATCH)
          echo "security_and_analysis.$security_field_name matches .csarc/policies/security-scanning.json (enabled)."
          ;;
        DEGRADED)
          [[ "${GITHUB_ACTIONS:-}" == "true" ]] &&
  ...
  if [[ "$apply_release_policy" != "true" ]]; then
    echo "SKIPPED .csarc/policies/releases.json (release_immutable_releases=$release_immutable_releases; not required by this release ownership)."
...
PY
  ...
  if [[ "$policy_actions_permissions" != "true" ]]; then
    echo "SKIPPED .csarc/policies/actions.json (policy_actions_permissions=false in .csarc/config.yml)."
  ...
  elif ! actions_comparison="$(python3 - "$repo_root/.csarc/policies/actions.json" "$actions_state" 2>&1 <<'PY'
import json
import sys
...
PY
    ...
    if [[ "$actions_default_drift" == "true" ]]; then
      echo "Actions settings drift: default_workflow_permissions differs from .csarc/policies/actions.json." >&2
      check_errors=$((check_errors + 1))
    fi
    if [[ "$actions_pr_drift" == "true" ]]; then
      echo "Actions settings drift: can_approve_pull_request_reviews differs from .csarc/policies/actions.json." >&2
      check_errors=$((check_errors + 1))
    elif [[ "$actions_pr_degraded" == "true" ]]; then
      [[ "${GITHUB_ACTIONS:-}" == "true" ]] &&
        echo "::warning title=Actions policy degraded::Actions cannot approve pull requests although .csarc/policies/actions.json requests it."
    ...
    if [[ "$actions_default_drift" != "true" && "$actions_pr_degraded" != "true" && "$actions_pr_drift" != "true" ]]; then
      echo "Actions workflow permissions match .csarc/policies/actions.json."
    fi
  fi

  if [[ "$policy_labels" != "true" ]]; then
    echo "SKIPPED .csarc/policies/labels.json (policy_labels=false in .csarc/config.yml)."
    ...
    echo "$labels_state" >&2
    check_errors=$((check_errors + 1))
  elif ! labels_drift="$(python3 - "$repo_root/.csarc/policies/labels.json" "$labels_state" 2>&1 <<'PY'
import json
import sys
...
PY
    ...
    echo "Policy labels match .csarc/policies/labels.json; extra labels are allowed."
  fi

  if [[ "$policy_branch_ruleset" != "true" ]]; then
    echo "SKIPPED .csarc/policies/rulesets.json (policy_branch_ruleset=false in .csarc/config.yml)."
    ...
    if [[ "$ruleset_inventory_available" != true ]]; then
      ...
      echo "MISSING remote Ruleset: desired state is preserved in .csarc/policies/rulesets.json."
...
if [[ "$policy_repository_settings" == "true" ]]; then
  echo "- APPLY .csarc/policies/repository.json"
else
  echo "- SKIP .csarc/policies/repository.json (disabled by policy_repository_settings=false)"
fi
echo "- APPLY .csarc/policies/issue-creation.json (issue_creation_policy via GraphQL)"
if [[ "$apply_release_policy" == "true" ]]; then
  echo "- APPLY .csarc/policies/releases.json (immutable Releases)"
else
...
if [[ "$policy_actions_permissions" == "true" ]]; then
  ...
  echo "- SKIP .csarc/policies/actions.json (disabled by policy_actions_permissions=false)"
fi
echo "- APPLY .csarc/policies/security-scanning.json (security_and_analysis) when GitHub Advanced Security or repository visibility permits it"
if [[ "$policy_labels" == "true" ]]; then
  echo "- APPLY .csarc/policies/labels.json (create or update policy labels)"
else
  echo "- SKIP .csarc/policies/labels.json (disabled by policy_labels=false)"
fi
if [[ "$policy_branch_ruleset" == "true" && "$legacy_ruleset_id" != "-" ]]; then
...
if [[ "$policy_labels" == "true" ]]; then
  ...
  desired_labels="$({
    uv run --no-project python - "$repo_root/.csarc/policies/labels.json" <<'PY'
import json
import sys
...
if [[ "$policy_branch_ruleset" != "true" ]]; then
  echo "- SKIP .csarc/policies/rulesets.json (disabled by policy_branch_ruleset=false)"
elif [[ "$ruleset_enforcement_available" == true ]]; then
  echo "- APPLY .csarc/policies/rulesets.json + .csarc/policies/rulesets-required-checks.json (enforced by GitHub)"
  echo "CODEOWNERS team: $code_owner"
elif [[ "$ruleset_inventory_available" == true ]]; then
  echo "- PRESERVE .csarc/policies/rulesets.json locally (public APIs cannot create a Ruleset on this plan)"
  ...
  echo "- PRESERVE .csarc/policies/rulesets.json locally (cannot inspect manually staged Rulesets: $graphql_ruleset_error)"
  print_ruleset_guidance
fi
...
if [[ "$policy_actions_permissions" == "true" ]]; then
  if ! actions_policy_error="$(
    gh api --method PUT "repos/$repo/actions/permissions/workflow" \
      --input "$repo_root/.csarc/policies/actions.json" 2>&1
  )"; then
...
if [[ "$policy_branch_ruleset" == "true" && "$ruleset_enforcement_available" == true && "$actions_policy_applied" == true &&
