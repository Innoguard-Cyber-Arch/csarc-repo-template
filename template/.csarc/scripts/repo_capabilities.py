...
`.csarc/scripts/apply-repository-settings.sh` already probes what a GitHub
*plan* (Free/Team/Enterprise) allows before applying
`.csarc/policies/*.json`, and prints a `DEGRADED` marker when a paid or
organization-policy-gated capability is
...
on top of it: a declarative matrix (`.csarc/policies/capability-matrix.json`)
...
`.csarc/scripts/check-repo-capabilities`) the live fact-gathering that feeds it.

Facts are supplied as a JSON object keyed by capability id, each value
`{"status": "allowed" | "blocked" | "unknown", "detail": "<short note>"}`.
`.csarc/scripts/check-repo-capabilities` gathers these from live `gh api` calls;
...
DEFAULT_MATRIX_PATH = (
    REPO_ROOT / ".csarc" / "policies" / "capability-matrix.json"
)
...
def load_matrix(path: Path) -> list[JsonObject]:
    """Load and minimally validate `.csarc/policies/capability-matrix.json`.
...
def _main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        ...
        help="Path to .csarc/policies/capability-matrix.json.",
    )
    parser.add_argument(
        "--facts",
        required=True,
