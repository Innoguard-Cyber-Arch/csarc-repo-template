...
Both `.csarc/scripts/check-repo-site-translations` (structural shape
comparison of the two language sources) and `.csarc/scripts/build_repo_site.py`
(the rendering engine) need to recognize the same Hugo-shortcode-style block
syntax still used, unchanged, by `docs/site/content/_index.{zh-tw,en}.md`. This
...
# Reused verbatim by .csarc/scripts/check-repo-site-translations for its own
...
# in .csarc/scripts/build_repo_site.py). Stripping them before the "is this
...
# `{{< /name >}}` close tag; docs/site/content/*.md never nests one inside
