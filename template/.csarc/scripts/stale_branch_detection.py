...
Issue #667. `delete_branch_on_merge` (`.csarc/policies/repository.json`) already
deletes a branch's remote copy automatically the instant its pull request
merges -- see `.csarc/docs/ci-policy.md`'s "合併後自動刪除一般來源 branch"
note. The one case it never reaches is a pull request that is **closed without
...
state):
  ...
  - `dev/m<N>-<slug>` -- Milestone delivery branches (the same shape
    ...
    documented in `.csarc/docs/ci-policy.md`
  - `csarc/*` -- machine-managed infrastructure refs, not work branches:
    `csarc/leases/*` (`.csarc/scripts/pr_lifecycle.py` PR lifecycle leases) and
...
standalone scheduled workflow):
  - `.csarc/scripts/sync_milestone_state.py preflight` -- a
    review-candidate list surfaced every time Milestone metadata is validated.
  - `.csarc/scripts/release_policy.py preflight` -- the same list surfaced as
...
def _cli_main(arguments: list[str] | None = None) -> int:
    ...
    this locally, mirroring the pattern of most other `.csarc/scripts/*.py`
