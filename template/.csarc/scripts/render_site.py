...
# .csarc/scripts/build_repo_site.py in Issue #524) must still close instead of
...
def _load_config(root: Path) -> dict[str, object]:
    ...
    config_reader = root / ".csarc/scripts/csarc_config.py"
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--source", type=Path, default=Path("dist/repo-site/index.html")
    )
    ...
    if args.check:
        ...
        if current != bundled:
            sys.stderr.write(
                ...
                "`uv run --no-project python .csarc/scripts/render_site.py`.\n"
            )
            return 1
        return 0
