...
def evidence(
    *,
    tier: str = "fast",
    command: str = "./.csarc/scripts/verify-fast",
    tree: str = TREE,
    ...
    if sync is not None:
        ...
        evidence_step = (
            ...
            f"tree={tree} command=./.csarc/scripts/verify-fast base={BASE} "
...
@pytest.mark.parametrize(
    ("tier", "command"),
    [
        ("docs", "./.csarc/scripts/verify-fast"),
        ("fast", "./.csarc/scripts/verify-fast"),
        ("full", "./.csarc/scripts/verify"),
    ],
)
...
def test_generated_full_verifier_is_explicit() -> None:
    ...
    check_run, workflow_run, job = evidence(
        tier="full", command="./.csarc/scripts/verify"
    )
    result = validate_verification_job(
        check_run,
        workflow_run,
    ...
    assert result["command"] == "./.csarc/scripts/verify"
