...
`.csarc/policies/rulesets-required-checks.json`（與 `template/.csarc/policies/rulesets.json.jinja` 的 `required_status_checks`）
...
`title` job同檔、同觸發條件，`template/` 同步一份）補上這個缺口，呼叫新增的 `.csarc/scripts/promotion_gate.py
check-route` 子指令。
...
`python3 .csarc/scripts/promotion_gate.py --help | grep -q check-route` 探測 base commit 上是否已經有這個子
...
這個 job **不**取代既有的 Milestone 交付驗證：`title` job（`.csarc/scripts/validate-pr-policy`）仍然負責 tracker
...
齊全」——後者仍由 `.csarc/scripts/promotion_gate.py` 的 `prepare()`／`finalize()` 在正式的 Milestone 交付流程中
處理，範圍不變。
...
2. **日常 PR gate（`docs`／`fast`，同一個成本邊界）**——`.csarc/scripts/ci_tier.py` 依事件、
   base／head、labels 與 changed paths 做 fail-closed 分類；未知或高風險內容升級為
   full。純文件／site 變更落在 `docs`，是 `fast` 的 early-exit 實作細節，不是獨立的第
   四套政策；其餘一般變更落在 `fast`。兩者入口都是 `.csarc/scripts/verify-fast`。本機可在需要廣泛
...
| Static assets and paired files | `.csarc/scripts/verify-stage-static-assets` | repo-site 可重現 render、workflow／shell 靜態分析、static-validation fixture 的正／反向覆蓋、root／template 配對檔案漂移 | fast 只在對應 scope 才跑其中個別項目（`docs` tier 跑 render 檢查；`workflow`／`shell` scope 才跑 lint）；full 一律跑全部四項，是唯一同時驗證全部四種風險的入口 |
