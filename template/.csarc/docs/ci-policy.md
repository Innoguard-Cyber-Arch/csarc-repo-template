...
`.csarc/scripts/check-governance-drift` 供本機驗證，不另外啟用排程。
...
release-worthy PR 都在原 PR 的 final release-only commit materialize 精確版本與
CHANGELOG；Milestone work 使用 beta，promotion、standalone 與 hotfix 使用 stable。
`promote/m<編號>-<簡稱>` PR 以
...
`.csarc/scripts/gh-issue-edit` 取代直接呼叫 `gh issue edit`：

```bash
.csarc/scripts/gh-issue-edit 740 --body-file tracker.md
```
...
Agent 先用 `.csarc/scripts/pr_lifecycle.py create` 從已 push、乾淨的 exact head 直接建立
Draft PR；不先建立 Ready PR 再切回 Draft，因此反覆修改不會啟動 hosted runner。後續若要
變更 ready／draft、授權或 metadata，必須先取得 remote lease，並透過
`.csarc/scripts/pr_lifecycle.py` 執行；`.csarc/scripts/verify` 會拒絕另一套重複寫入者。
人工在 GitHub 上審查與合併不受這個工具限制。

### Exact-head review 即為合併授權（#719）

一般有獨立 reviewer 的 PR，不需要再留一則重複的授權留言。`.csarc/scripts/pr_lifecycle.py`
...
- `copilot_review: allowed`：Ruleset 要求 0 個原生 approval，改由
  ...
  `review` 列為 required check（`.github/workflows/pr-review.yml` → `.csarc/scripts/review_gate.py
  ...
  3.（#775／#826／#905／#918）這是一張符合 `admin_bypass_opt_in` 條件（PR body 恰好一次
...
1. `python3 .csarc/scripts/review_gate.py status --repo <owner/repo> --pr <N>` 取得 Copilot 對目前
...
4. 依上方 single-writer 規則取得 lease，`.csarc/scripts/pr_lifecycle.py merge` 以
...
管理員執行 `./.csarc/scripts/apply-repository-settings.sh plan`／`apply`／`check`。
...
1. `pr-policy.yml` `title` job 的「Validate Milestone approval」step（要求非提案者在
   ...
   的 conclusion 就一併略過——用 #513 的 `.csarc/scripts/check-pr-policy-status`（完成前，
   改用 `gh run view <run-id> --log | grep -E "Validate pull request policy|##\[error\]"`
   手動確認）。
2. Ruleset 的 self-approval 結構性卡點，見下方「Beta／stable 發布通道與 admin
   bypass」。

**`--admin` 本身不足以繞過任何 Ruleset 規則。** 舊版 classic branch protection 會自動
給 repository admin 身分繞過，但 Ruleset 只認 `.csarc/policies/rulesets.json`（或本節後述
...
Alpha PR 可由 `.csarc/scripts/pr_lifecycle.py merge` 在 lease＋exact-head 授權留言齊全後直接
...
`/milestone admin-approve` 自核（見 `.csarc/docs/milestone-description.md`）是另一個獨立機制，
...
`.csarc/policies/rulesets.json` 加上等效項目。
...
同行核准。生成專案預設 `off`，可明確選擇另外兩種模式。無論設定值為何，bypass
都不能略過 release-level/path-risk 選出的 suite、required checks、候選版本檢查、
remote lease、即時 admin 身分或 exact-head 綁定。實際使用由 lifecycle 寫入：
...
* `.csarc/policies/rulesets-required-checks.json` 只放 required checks，
  `bypass_actors` 永遠是 `[]`。
* `.csarc/policies/rulesets.json` 放 `non_fast_forward` 與 `pull_request`；admin
...
> 本節以下僅保留歷史設計脈絡。#745 已刪除 `.csarc/policies/project-stage.json`
...
`.csarc/policies/project-stage.json`（`{"release_phase": "alpha"}`），只有三個合法值：
`alpha`／`beta`／`release`。它跟本 repo 既有兩個外形相似但軸線不同的「stage」概念
刻意分開，避免第三次命名碰撞——`.csarc/scripts/generate_audit_trail.py` 的
...
`.csarc/scripts/release_phase_rulesets.py` 的 module docstring。跟 `profiles/catalog.yaml`
...
* `.csarc/policies/rulesets.json`（"CSARC protected branches"）——`non_fast_forward` ＋
  `pull_request` 規則，`bypass_actors` 帶上述 admin 角色項目。
* `.csarc/policies/rulesets-required-checks.json`（"CSARC required checks"）——只有
  `required_status_checks` 規則，`bypass_actors` 永遠是 `[]`。

`.csarc/scripts/release_phase_rulesets.py`（`apply-repository-settings.sh` 呼叫它的
...
`.csarc/scripts/apply-repository-settings.sh check` 的既有 drift 比對（比較
`gh api repos/{repo}/rules/branches/{branch}` 回傳的「該分支目前有效的規則聯集」）
也相應改為比較兩個檔案 `rules` 的聯集，不管哪個規則實際放在哪個 Ruleset 物件裡。

**release 階段的自動失效是結構性保證，不是人工步驟**：`.csarc/scripts/check-bypass-lifecycle`
（已接進 `./.csarc/scripts/verify-fast`，每次 PR 都跑）讀取 `.csarc/policies/project-stage.json`，
只要 `release_phase` 是 `"release"`，`.csarc/policies/rulesets.json` 或
`.csarc/policies/rulesets-required-checks.json` 裡有任何非空 `bypass_actors`，就直接
...
`.csarc/scripts/check-bypass-trace <PR 編號> --repo <owner/repo>`（核心比對邏輯在
`.csarc/scripts/check_bypass_trace.py`，回歸測試在 `tests/test_check_bypass_trace.py`）
...
`.csarc/scripts/generate_audit_trail.py` 已在抓這些欄位）目前不在這個查核工具範圍內：
...
`.csarc/scripts/check-pr-policy-status` 的用法一樣是針對單一 PR 主動查核，不是排程掃描。
...
`/milestone admin-approve` 自核（見 `.csarc/docs/milestone-description.md`）是另一個獨立
...
不帶 `.csarc/policies/project-stage.json` 或第二個 Ruleset 檔，只有真的撞上同一個「結構性
只有一個真人帳號」問題的下游 repo，才需要自行決定是否套用等效機制（沿用 #580 已
落地的判斷）。`.csarc/scripts/apply-repository-settings.sh` 對這兩個新政策檔案的存在與否
是條件式判斷：檔案不存在時（所有既有下游 repo）行為與本 Issue 之前完全一致。

`.csarc/scripts/pr_lifecycle.py` 的 `scan_writers`（`command_writer_violations`／
`declarative_writer_violations`）掃描 `.github/workflows/`、`.csarc/scripts/`
...
一個 block 比對，導致 `.csarc/scripts/gh-issue-create` 裡兩句不相干的 `#` 註解（一句
提到 `` `gh issue edit` ``、另一句列出 `--milestone` 這個透傳 flag）被誤判成
「gh issue metadata write」；`.csarc/scripts/validate-pr-policy`（#551 的 Milestone
...
一般工作 branch 由 `.csarc/scripts/gh-issue-develop` 建立；工具在呼叫 GitHub 前要求名稱符合
...
`release-recovery` 標籤（`.csarc/policies/labels.json`）標出「`main` 缺少一次應有發版、需要直接對
`main` 提出稽核過的修正」這條路徑，與 hotfix 結構相近但目的不同：hotfix 修正 `main` 上的
缺陷本身，release recovery 修正「發版流程沒有正確完成」這件事。`.csarc/scripts/promotion_gate.py`
的 `route_for()` 只在分支符合 `fix/<Issue>-<slug>`、PR 標題型別為 `fix`、target `main`，且
**沒有**同時掛 `hotfix` 標籤時，才把掛了 `release-recovery` 標籤的 PR 分類為
`release-recovery` route；`.csarc/scripts/validate-pr-policy` 對同一組條件做本機可重跑的驗證，違反
任一條就擋下合併。`.csarc/scripts/ci_tier.py` 讓這條路徑比照 hotfix 一律升級為 `full` 驗證分級，不
...
- 先比對 PR 的 head branch 是否落在 `_AUTOMATED_PULL_REQUEST_HEAD_PREFIXES`
  ...
  `.csarc/scripts/validate-pr-policy` 既有的四種自動化路徑一致），符合就直接放行，不看 body
  內容。這是防禦自動化 PR body 內文「意外像」`Fixes #N` 的假陽性——例如 Dependabot
  把上游 changelog 逐字帶進 PR 說明，剛好包含這個字串。這不是「只要是 bot 帳號就跳
  過」的寬鬆規則（見 `.csarc/scripts/hosted_verify_bots.py` 自己「不是只要 `[bot]` 就跳過」
...
- 再用 `_CLOSING_KEYWORD.finditer()`（不是 `.search()`）收集 body 裡**所有**相異的
  ...
  每個編號，不會像只用 `.search()` 那樣悄悄只看第一個、漏掉其餘。`.csarc/scripts/
...
- 這裡的 pattern 在形狀上接近、但不是逐字等於 `.csarc/scripts/check-scope-gate` 自己的
...
（`.csarc/scripts/validate-pr-policy` 已經要求 PR 的 Milestone 必須與其連結 Issue 的
...
Milestone 的情境），呼叫新增的 `.csarc/scripts/sync_milestone_state.py
...
權限與生命週期，而且唯一額外工作只有 `.csarc/scripts/promotion_gate.py check-route`。因此 route
classifier 現在是 `title` job 內的一個 step，Ruleset 不再要求第四個 `promotion` context。這省掉
一個 runner job，但沒有刪除 route 判定。
...
的 `Milestone approval` 是 comment refresh 用的非 Ruleset check，由上游模板另行維護。
...
| Milestone comment refresh | 非 Ruleset 的 `Milestone approval` check 只處理核可留言後重新計算 | 由上游模板獨立維護；不與 required context 重複 |
...
的目標不變。`.csarc/scripts/verify-fast`、`.csarc/scripts/verify-template.sh`（生成 repo 是 `.csarc/scripts/verify`）仍是本機與
...
工具鏈，依可信 plan 實際執行 `.csarc/scripts/verify-fast` 或 full verifier。Dependabot 與一般 contributor 共用同一路徑，
不再有 bot 白名單。

`verify` 的 job-level guard 在 runner 排程前只排除 Draft PR 活動。轉成 Ready、非 Draft 的新 head／`edited`、
任何 `labeled`／`unlabeled`、merge queue 與手動執行都會啟動並回報名為 `verify` 的 check。這個 guard 只節省非必要
runner，不產生可信成功證據，也不改變下方 exact-head 驗證條件。

非 tier label 不進入 evidence identity，所以同一 head 的 label 事件會走「Find reusable trusted verification」，
直接引用原本 Execute 的結果，不重跑整套驗證；`promotion`／`hotfix`／`release-recovery` 會改變 identity，因此重算
tier 並執行。#1019 之前，非 tier label 會讓 job 在 guard 被略過：GitHub 把最新那次 CI run 當成 head 的結果，
裡面卻沒有 `verify`，於是 required check 回報 expected 並擋下 merge，即使同一 head 早先的 `verify` 已經成功。

`.csarc/scripts/check-trusted-verification` 與 `.csarc/scripts/verification_evidence.py` 消費 GitHub Check Runs、Actions run 與
Jobs API，並重用 `.csarc/scripts/pr_lifecycle.py` 的 required-check producer selector。有效證據必須同時符合：
...
與存在的同名 `.csarc/scripts/foo.py → tests/test_foo.py`；再依 source、template、governance、workflow、shell 加上固定的
...
| 非 Draft `labeled`／`unlabeled` | 只有事件本身變動 `promotion`／`hotfix`／`release-recovery` 才啟動 required `verify` 並重算 tier；其他 label 也回報 `verify`，以同-head reuse 引用原本的 Execute，不重跑驗證（#1019） |
...
回歸測試集中在 `.csarc/tests/test_verification_evidence.py`、`tests/test_pr_lifecycle.py`、
...
`do_not_enforce_on_create: true`（`.csarc/policies/rulesets-required-checks.json`；生成 repo 對應
...
`.csarc/scripts/apply-repository-settings.sh` 的 drift-check 原本只比對 `required_status_checks` 的 context 清
...
`.csarc/scripts/release_phase_rulesets.py assemble` 的既有輸出（純函式，不需要 live API）。live 分支建立本身無法
純單元測試（需要真的對 GitHub 送出請求），驗證步驟改為文件化的手動步驟：管理員執行
`./.csarc/scripts/apply-repository-settings.sh apply` 套用新設定後，對一個目前不存在、符合 `dev/m*` pattern 的分
...
1. `.github/workflows/dependabot-auto-merge.yml`（root 與 `template/.github/workflows/dependabot-auto-merge.yml`，
   ...
   `.csarc/scripts/pr_lifecycle.py acquire → check → merge` 重新驗證 live PR、Bot account type、同 repo ref、current commit
...
2. 新增 `.csarc/scripts/check_action_pins.py`（root 與 `template/.csarc/scripts/check_action_pins.py` 逐位元組同步）：掃
   ...
   `.jinja` 這塊 Dependabot 結構性掃不到的缺口。掛進 `.csarc/scripts/verify-fast`（`workflow` scope 時執行）與
   `.csarc/scripts/verify-stage-github-actions-audit`（`verify-template.sh` 的一部分，跟 zizmor 同一階段）。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CI | `.github/workflows/ci.yml` | 驗證分級（#392／#403／#428／#812）；可信 hosted execution（#834）；runner guard（#901） | `pull_request_target`、`merge_group`、`workflow_dispatch`；Draft 活動由 job guard 在 runner 前排除，Ready、非 Draft code／edited 與所有 label 事件都回報 `verify`（非 tier label 走同-head reuse，#1019） | `actions`／`checks`／`contents`／Issues／PR read；30 分鐘；同一 PR 新 commit 取消舊 run | base-trusted `.csarc/scripts/ci_tier.py` 分類後，在 GitHub-hosted `ubuntu-latest` 對 exact candidate 執行 risk-owned `.csarc/scripts/verify-fast`／full verifier；同 head 的 `edited`／tier-label run 可單跳引用相同 route 的原始 Execute，且只有 `promotion`／`hotfix`／`release-recovery` 三種 label 會啟動 required `verify` 並納入 evidence identity；其他 label 的 skipped check 使用非 required 名稱，不能覆蓋既有 `verify` 結果；clean main-to-delivery sync 先做結構預檢再引用 current main 的 fresh full Execute；所有證據綁定 repository、head/tree、base、tier/scopes、command、toolchain、tier labels、release level、runner、result 與 24 小時 freshness | `tests/test_ci_tier.py`；`tests/test_journey03_ci.py`；`.csarc/tests/test_verification_evidence.py`；`tests/test_pr_lifecycle.py`；`tests/test_delivery_sync.py` | #834 hosted execution active；#812 risk-owned/reuse/sync route active；#901 guard 待首次 Ready／label 事件 live evidence |
| PR policy | `.github/workflows/pr-policy.yml` | PR／交付政策 | `pull_request_target` PR metadata 事件（opened／edited／synchronize／labeled）、`merge_group` | `contents`／Issues／pull requests 只讀；固定 timeout | 單一 `title` job：draft 期間不啟動 runner，`ready_for_review` 後才完整驗證 Issue、route、review policy、promotion route 與 Milestone approval，以原生 job conclusion 回報結果 | `.csarc/scripts/test-pr-policy`；`tests/test_journey05_workflows.py`；route classifier 見 `tests/test_promotion_gate.py` 的 `test_check_route_*` | 歷史 run [33519320929](https://github.com/Innoguard-Cyber-Arch/csarc-repo-template/actions/runs/33519320929) 證明既有 policy 判定；上游 #876 合併後補 live consolidated-job evidence | 既有 policy：active；#876 consolidation：candidate |
...
| PR review（Copilot 審核模式，#752／#775／#826／#900／#926／#933） | `.github/workflows/pr-review.yml` | PR 審核授權 | `pull_request_target`（opened／synchronize／reopened／ready_for_review）、`pull_request_review`（submitted／dismissed）、`issue_comment`（created，篩選 PR 上以 `PR lifecycle merge authorization` 開頭的留言）、`merge_group` | publisher 取得 `checks: write` 與必要讀權限；10 分鐘；同 PR 新事件取消舊 run；draft PR 一般事件不啟動 runner | trusted publisher 呼叫 `.csarc/scripts/review_gate.py publish`，把等待、通過與確定拒絕分別發布為 `review` 的 queued／success／failure；publisher 只有判定或 API 寫入失敗才失敗。merge queue 沿用已通過的 PR head review | `tests/test_review_gate.py`；`.csarc/scripts/pr_lifecycle.py` 的授權來源與 custom check provenance 見 `tests/test_pr_lifecycle.py` | #900 candidate | candidate |
...
| Release publish drift alert | `.github/workflows/release-drift.yml` | #605／#933（源自 #589 item 4） | daily schedule＋`workflow_dispatch`（`hours` input） | `actions: read`、`contents: read`、`issues: write`；5 分鐘 | 偵測到 drift 時開立或更新追蹤 Issue 後成功；未偵測到時只印出證據；偵測或通知寫入失敗才讓 run 失敗 | `.csarc/scripts/test-check-release-drift` | 尚未 merge 進 `main`，故無排程或手動觸發的 live run 證據 | candidate（待 main 落地＋首次排程／手動觸發） |
...
timeout，只呼叫 `.csarc/scripts/check-template-update`）。公開模板來源不需要 secret；
...
```console
python3 .csarc/scripts/sync_milestone_state.py preflight --repo <owner>/<repo> --milestone <N>
```
...
focused check。最後要求是發布層級下限與 `.csarc/scripts/ci_tier.py` 路徑分類取較強者。
`.csarc/scripts/verify-fast` 從 `branch.<name>.gh-merge-base`（必要時以 `CSARC_CI_BASE` 指定）找出 PR base，
...
既有設定中的 `baseline`／`docs` 在 update migration 與 runtime 都會正規化為 `fast`；新設定只提供
...
1. **開發中 focused check（本機專用，不是 CI 的第三種政策）**——實作中與一般 push 前直接執行
   ...
   `.csarc/scripts/verify-template.sh` 其中一階段單獨重跑對應的
   `.csarc/scripts/verify-stage-<name>`；不需要等待整條 pipeline，也不會被當成合併證據。
2. **日常 PR gate（`fast`）**——`.csarc/scripts/ci_tier.py` 依事件、base／head、labels 與
   changed paths 做 fail-closed 分類；未知或高風險內容升級為 full。純文件／site 變更仍是
   `fast`，由 docs scope 只啟動對應 owner，不再建立一個只有名稱不同的 `docs` tier。
   入口是 `.csarc/scripts/verify-fast`。本機可在需要廣泛
...
| full | stable | fast owners ＋ `large` 的代表 create／adopt／previous-release update（create 含 Rust 與 Go native）與 coverage | 目標不超過 15 分鐘（#812／#918） |
...
1. **這個 branch 已經對自己這一輪真正的內容，跑過一次全綠的本機
   `./.csarc/scripts/verify-template.sh`（或生成 repo 的 `./.csarc/scripts/verify`）。** own-verified-
...
4. **上游帶進來的變更沒有觸碰驗證／CI／政策基礎設施本身**——`.csarc/scripts/verify*`、
   `.github/workflows/`、`.csarc/scripts/ci_tier.py`、`.csarc/scripts/pr_lifecycle.py`、
   `.csarc/scripts/validate-*-policy`、`.csarc/scripts/test-issue-triage`、
   `.csarc/scripts/test-worktree-cleanup`、`.csarc/scripts/test-pr-policy`、`.csarc/policies/` 等。就算與這
...
`./.csarc/scripts/verify-template.sh`，改為信任 hosted `verify` check 作為這次重新合併後的最
終驗證。這**不代表**降低驗證涵蓋範圍：hosted CI 對這次合併結果執行的仍是同一支
`.csarc/scripts/verify-template.sh`／`.csarc/scripts/verify-stage-*`（生成 repo 是同一支
`.csarc/scripts/verify`），差別只在「誰的環境跑」從本機換成乾淨、不受本機 worktree 並行負載或
...
`.csarc/scripts/test-check-base-only-remerge` 對合格案例與三種不合格案例（檔案重疊、觸及驗證
基礎設施、檔名不重疊但仍衝突）做回歸測試，掛在 `.csarc/scripts/verify-template.sh` 的
Regression tests 階段下（與 `test-issue-triage`／`test-worktree-cleanup`／
`test-pr-policy` 同一組 self-test）。它不掛在 `.csarc/scripts/verify-fast`：`verify-fast` 的
...
`.csarc/scripts/check-base-only-remerge` 只存在於中央模板 repo、不會下發到生成 repo（生成
repo 的 full 入口是單一 `.csarc/scripts/verify`，沒有本模板這種多階段重新合併場景），所以只加
...
--check` 與 `.csarc/scripts/check-update-conflicts` 兩者都乾淨），並確認上游變更沒有觸及上方
條件 4 列出的路徑。
...
`.csarc/scripts/validate-pr-policy`，其中包含兩個 acceptance-checklist 檢查——PR 本文自己
...
`.csarc/scripts/validate-pr-policy` 現在讀取 `PR_DRAFT`（`.github/workflows/pr-policy.yml`
...
`.csarc/scripts/test-pr-policy` 新增回歸案例，涵蓋：`PR_DRAFT=true` 時，PR 本文與連結
...
`.csarc/scripts/check-pr-policy-status <pr-number> [--repo <owner/repo>] [--json]`
...
`.csarc/scripts/test-check-pr-policy-status` 用 mock `gh`（不打真實網路）對「job
整體 fail 但目標 step success」與「目標 step 真的 fail」兩種情況各自回歸
測試，也涵蓋 `gh` 未安裝與 PR 不存在兩種誤用場景，掛在
`.csarc/scripts/verify-template.sh` 的 Regression tests 階段下（與
`test-check-base-only-remerge` 同一組 self-test）。`.csarc/scripts/check-pr-policy-status`
...
- **CI 接線：**`pr-policy.yml` 的 `title` job 新增「Validate the scope-drift
  ...
  之前執行，呼叫新腳本 `.csarc/scripts/check-scope-gate`。這支腳本從 PR body 解析
  `Closes|Fixes|Resolves #<n>`（與 `.csarc/scripts/validate-pr-policy` 自己用來核對
  ...
  policy 腳本），`.csarc/scripts/check-scope-gate` 這支新腳本要等到落地 `#632` 的
  這次 PR 本身合併進 base branch 之後才會出現在該 checkout 裡；因此 step 先
  判斷腳本是否存在（`[[ -x ./.csarc/scripts/check-scope-gate ]]`），不存在只印
...
`.csarc/scripts/test-check-scope-gate` 對 `check-scope-gate` 本身做端到端回歸測試
（無連結 Issue、無 sentinel、有 sentinel 未核可、有 sentinel 已核可、核可過期
四種結果，掛在 `.csarc/scripts/verify-stage-regression-tests`）。routine fast 由同一 Python suite
...
### `.csarc/scripts/verify-template.sh` 階段盤點（#458）

`.csarc/scripts/verify-template.sh` 是一個薄聚合器：七個階段各自是 `.csarc/scripts/verify-stage-*`
底下一支可獨立執行的腳本，聚合器仍依相同順序呼叫，並保留原有的
START／PASSED／FAILED／timing summary 輸出格式與 pass/fail 語意；
`.csarc/scripts/test-verify-template-stages` 對這個聚合契約（呼叫順序、腳本存在且可執行、
PASSED／FAILED／TOTAL 回報）做回歸測試，並同時掛在 `.csarc/scripts/verify-fast` 的
`shell` scope 與 `.csarc/scripts/verify-template.sh` 的 Regression tests 階段下。

盤點結論：七個階段各自覆蓋不重疊的風險，且 `.csarc/scripts/verify-fast` 對同一批底層腳本只是
...
| --- | --- | --- | --- |
| Repository contracts | `.csarc/scripts/verify-stage-repository-contracts` | changed-tree hygiene、未解決的 Copier／Git 衝突標記、機密掃描、已知漏洞掃描 | fast 每次都跑 `git diff --check`／`check-update-conflicts`／`scan-secrets`；`verify-dependencies` 只在 dependency scope 才跑，呼叫同一支腳本，不是重複邏輯 |
| Static assets and paired files | `.csarc/scripts/verify-stage-static-assets` | repo-site 可重現 render、workflow／shell 靜態分析、static-validation fixture 的正／反向覆蓋、root／template 配對檔案漂移 | fast 只在對應 scope 才跑其中個別項目（`docs` scope 跑 render 檢查；`workflow`／`shell` scope 才跑 lint）；full 一律跑全部四項，是唯一同時驗證全部四種風險的入口 |
| Python environment | `.csarc/scripts/verify-stage-python-environment` | `uv.lock` 與 `pyproject.toml` 一致、環境可從鎖定版本安裝 | fast 的 `uv sync --locked` 是同一份鎖定契約；`uv lock --check` 只在 full 額外執行 |
| Python quality | `.csarc/scripts/verify-stage-python-quality` | 格式、lint、靜態型別 | fast 對相同原始碼跑相同三個命令，兩者呼叫同一份工具鏈設定，無額外邏輯 |
| Regression tests | `.csarc/scripts/verify-stage-regression-tests` | 完整 pytest（含 `large` 標記的 Copier create／existing-adoption／update 保存回歸）＋coverage 門檻，以及 Issue-triage／worktree-cleanup／PR-policy／scope-drift-gate／base-only-remerge／gh-issue-create／gh-issue-develop／check-branch-fresh／PR-policy-status／release-drift／audit-fleet-adoption／create-milestone／`verify-template.sh` 聚合自我測試 | fast 只跑 `pytest -m "not large"`（略過 `large`）；較長的 shell lifecycle fixtures 留在 full，routine fast 由同一 Python suite 的窄版 policy 測試涵蓋。base-only-remerge、`.csarc/scripts/gh-issue-create`（開 Issue 前本機先擋不合規標題，見 AGENTS.md 工作迴圈）、`.csarc/scripts/gh-issue-develop`（建立遠端 ref 前擋不合規分支，只在尚無 open PR 時修復錯名，見 #932）、`.csarc/scripts/check-branch-fresh`（開工前本機核對既有分支是否仍等於 `origin/<branch>`，見 AGENTS.md 工作迴圈）、PR-policy-status、`.csarc/scripts/audit-fleet-adoption`（本機即時查詢 fleet 採用門檻、只印 stdout，見 #521）與 `.csarc/scripts/create-milestone`（原子建立 Milestone 與其 tracker Issue，見 `.csarc/docs/milestone-description.md`；#572）七支本機專用工具的自我測試都只在這個 full 專屬階段跑，不進 `verify-fast`（分別見上方 Base-only re-merge 例外一節與下方 PR policy 逐 step 判讀一節）；`.csarc/scripts/test-check-release-drift`（mock `gh`，驗證上方「發版存量漂移偵測（`release-drift.yml`，#605）」一節的 drift 判定邏輯）也只掛在這個 full 專屬階段——`.csarc/scripts/check-release-drift` 本身像 `release.yml` 一樣逐位元組下發到 `template/`，但比照 `release.yml`／`ci.yml` 沒有生成 repo 端本機再測試的既有慣例（下發前的 root 測試已足夠證明這份靜態、無 Jinja 條件式的實作正確），不隨腳本一起下發、也不掛進生成 repo 的 `.csarc/scripts/verify`；`large` 覆蓋範圍只在 full 執行，是 Copier create／adopt／update 保存的唯一 regression source，未被任何字串比對或重複 profile 執行取代 |
| Package smoke test | `.csarc/scripts/verify-stage-package-smoke` | wheel 可建置、已發布入口可從建置產物執行 | fast 不跑這個階段，也不執行真實 Copier；由 full 的代表 create canary 與本階段證明 |
| GitHub Actions audit | `.csarc/scripts/verify-stage-github-actions-audit` | workflow 權限與注入稽核（zizmor） | root `verify-fast` 在 workflow scope 直接執行；full 仍執行同一支 stage，不依賴未啟用的 merge queue 或後續 promotion 補跑 |
...
- **One representative end-to-end profile：**full regression 只挑一個 Python＋TypeScript＋Rust＋Go 組合；
  生成 repo 用 `.csarc/scripts/verify full` 一次涵蓋四種真實原生工具鏈，不再按語言重跑
...
所有本機驗證入口共用 `.csarc/scripts/verification-step`：每一步在開始與結束時印出名稱與 wall
time；執行超過 60 秒時每 60 秒輸出 heartbeat；失敗時先指出第一個失敗步驟、exit status、
`log=stdout`，再印出 shell-quoted 的單步 `RERUN` 指令。`.csarc/scripts/verify-template.sh` 仍維持
七階段摘要，並在 stage failure 另列可獨立重跑的 `.csarc/scripts/verify-stage-*` 指令。可用
...
`./.csarc/scripts/verify-template.sh`，前兩次都不能當作乾淨樣本——第一次在 Repository
...
| --- | --- | --- |
| Repository contracts | `.csarc/scripts/verify-stage-repository-contracts` | 10 |
| Static assets and paired files | `.csarc/scripts/verify-stage-static-assets` | 10 |
| Python environment | `.csarc/scripts/verify-stage-python-environment` | 2 |
| Python quality | `.csarc/scripts/verify-stage-python-quality` | 2 |
| Regression tests | `.csarc/scripts/verify-stage-regression-tests` | 3971 |
| Package smoke test | `.csarc/scripts/verify-stage-package-smoke` | 6 |
| GitHub Actions audit | `.csarc/scripts/verify-stage-github-actions-audit` | 1 |
| TOTAL | — | 4002 |

七個階段秒數總和（10+10+2+2+3971+6+1）等於同次執行回報的 TOTAL 4002 秒。

`.csarc/scripts/ci_tier.py` 另外輸出 `run_governance`／`run_osv`／`run_zizmor`／`upload_site`
四個欄位，供 plan 摘要與相容呼叫端使用；本機與 hosted `verify-fast` 直接以同一份 plan 的 `scopes`
調度子集，不維護第二套分類邏輯。

`.csarc/scripts/resolve-cache-root` 預設已經是使用者層級、跨 worktree 共用的位置（macOS 為
...
```bash
CSARC_CACHE_ROOT="$HOME/.cache/csarc" ./.csarc/scripts/verify-template.sh
```
...
- 先跑上方「`.csarc/scripts/verify-template.sh` 階段盤點（#458）」六個便宜階段（Repository
  ...
  `.csarc/scripts/verify-stage-<name>` 逐一單獨執行，不必等待整條聚合器。
...
- Regression tests 若因暫時性錯誤中止（例如 #465 記錄的
  ...
  `.csarc/scripts/verify-stage-regression-tests` 單獨重跑這一階段，不必連同已經 PASSED 的六個
  便宜階段一起重跑整支聚合器。
...
| 正式版本與 CHANGELOG | Milestone work PR 的 final release-only commit materialize beta | promotion bridge materialize stable | standalone／hotfix 原 PR materialize stable | manual 只重跑同一流程，不另開版本來源 |
| CI | fast／full 依風險 | 一律 full | release workflow 對目前 `main` 跑一次 full | 候選只跑版本／檔案／可打包 focused check；正式發布前已在 main 跑 full |
| 成品／checksum／SBOM | beta 合併後從精確 commit 建立 | stable 合併後從精確 commit 建立 | 同 PR stable 候選合併後建立 | draft Release 先上傳、下載重驗，成功才公開 |
| tag／GitHub Release | `X.Y.Z-beta.N` | `X.Y.Z` | `X.Y.Z` | 重跑只驗同一 tag；不移動 tag、不重寫成品 |
...
token 或空 deployment environment。所有候選都在原交付 PR 由 agent 產生並接受審查；
`release.yml` 合併後只發布，不使用 `GITHUB_TOKEN` 建立第二張版本 PR。
...
最高宣告層級——**宣告與計算機制由 #745 提供**，本節與 `.csarc/scripts/release_policy.py`／
`.csarc/scripts/publish-release`／`.csarc/scripts/converge-release-tag` 只負責把一個已宣告的層級
...
`.csarc/scripts/release_phase.py`
...
pre-release 列為應刪除。`.csarc/scripts/release_policy.py retention-plan --repo OWNER/NAME`
...
`.csarc/scripts/pr_lifecycle.py merge` 會在 remote lease 綁定的 current base 上重跑
`release_policy.py verify-delivery-version`，確認 head 的最後一個 commit 恰好是從前一個
implementation commit 產生的 release surfaces；`.csarc/scripts/publish-release stage` 在發布前
再以來源 PR 與 merge tree 重驗。Base 一旦前進，agent 必須先同步並重新 materialize，舊 base
的成功 status 不得滿足最終邊界（#817、#925）。

## Conditional 與退役能力

`.csarc/scripts/verify_release_consumption.py` 與其測試保留為 conditional 的消費端安全契約。
checksum 與 SPDX SBOM 已由 `.csarc/scripts/release_bundle.py` 納入 GitHub Release；production-side
...
- 合併後自動刪除一般來源 branch；Milestone／canary branch 等人工確認結案與 evidence 後
  刪除。`.csarc/scripts/cleanup-worktrees --apply` 只清除乾淨、未鎖定且可證明已合併的本機 worktree。
...
`delete_branch_on_merge`（`.csarc/policies/repository.json`）——這個設定只在 PR **合併**時
...
| `.csarc/scripts/apply-repository-settings.sh check`（既有，本節確認涵蓋） | 手動／CI 執行時 | `delete_branch_on_merge` 這個 repo 設定本身是否仍是 `true` | 只回報 drift，不刪除任何 branch |
| `.csarc/scripts/stale_branch_detection.py`（本節新增） | Milestone preflight／release preflight 執行時 | 關閉未合併、或從未開過 PR 的過時 branch | 只回報候選清單，**不刪除** |

**`apply-repository-settings.sh check` 的確認結果**：`delete_branch_on_merge` 已經是
`.csarc/policies/repository.json` 的既有欄位，`check` 子指令既有的 repository-settings drift
...
（`.csarc/scripts/test-apply-repository-settings` 的 Case 3b）。

**偵測邏輯（`.csarc/scripts/stale_branch_detection.py`）**：列出遠端 branch 中同時符合以下
...
或 `csarc/*`（機器管理的基礎設施 ref：`.csarc/scripts/pr_lifecycle.py` 寫入的
...
- `.csarc/scripts/sync_milestone_state.py preflight`：每次驗證 Milestone metadata 時，一併
  印出過時 branch review 清單（純提示，never 影響 `preflight` 本身的 pass/fail —
  這與這個 Milestone 的 metadata 是否就緒無關）。
- `.csarc/scripts/release_policy.py preflight`：與既有 `integrations`（Renovate 安裝建議）
...
`.csarc/scripts/apply-repository-settings.sh` 既有的 `DEGRADED` 機制回答的是「這個帳號的
GitHub 方案允許什麼」；但同一個方案上，organization 政策、CODEOWNERS team 是否存在、
token 權限範圍，仍可能個別擋住某一項能力——這一層目前沒有自動檢查，也沒有把 workaround
集中寫清楚。`.csarc/policies/capability-matrix.json` 補上這份「repo 能力矩陣」：每一項能力
...
偵測方式與已記錄的 workaround。`.csarc/scripts/repo_capabilities.py` 是純邏輯的 evaluator（三態
`allowed`／`blocked`／`unknown`，與 `docs/adr/capability-aware-governance.md` 既有的
三態慣例一致），`.csarc/scripts/check-repo-capabilities` 則是即時對這個 repo 探測、組成 facts
...
GitHub Pages 五項），就直接引用同一段既有訊息，而不是另建一套平行說法。`.csarc/policies/
...
**設計：** #589 的 Guided 名稱現在只表示「由維護者或 agent 在本機呼叫 publisher」，不再表示另開版本 PR。#925 要求所有候選都已在原交付 PR 內產生並通過同一 review；Actions／webhook 不健康時，只把合併後的發布步驟（建 tag、draft Release、build 成品、checksum、SPDX SBOM、`gh release` 系列指令）切到本機執行。`release.yml` 與本機路徑呼叫同一份 `.csarc/scripts/publish-release`，不維持兩套邏輯，也不能藉本機執行省略審查。

**代價（不能只講好處）：**

- **放棄 hosted runner 的乾淨、一致環境保證。** 本機執行的環境不由 GitHub 控管；只有本機 `full` 驗證全綠才能視為等同 hosted 的證明強度。
- **需要本機或執行者持有具備 admin／write 權限的長效憑證，而不是 Actions 短效 `GITHUB_TOKEN`。** 這不是為所有 CSARC-owned repo 新增一項標準要求——`.csarc/scripts/apply-repository-settings.sh apply` 本來就已經要求 repo admin 用自己的 `gh` 身分執行；本節只是讓同一位已經持有這個權限的維護者，多一個「用同一身分完成發版」的選項。
- **沒有 merge 後自動觸發，需要人或排程主動執行。** 需要另外一道獨立排程的存量檢查偵測「`main` 已經前進，但過去 N 小時內沒有明確的 `no-release` run 或有效的 immutable stable GitHub Release」，取代目前完全仰賴人工檢查 Releases 頁面才會發現的狀態；這道檢查已由 `.github/workflows/release-drift.yml`／`.csarc/scripts/check-release-drift` 落地，見下方「發版存量漂移偵測（`release-drift.yml`，#605）」一節。
...
`.csarc/scripts/converge-release-tag` 是唯一呼叫 `gh release create` 的地方：
...
`.csarc/scripts/publish-release stage`（hosted 與本機共用同一個進入點）呼叫這支腳本；
`release.yml` 與本機執行都呼叫同一份 `.csarc/scripts/publish-release`。版本與 CHANGELOG
已由原交付 PR 的 `release_policy.py prepare-candidate` 產生；合併後不再執行 Release Please
或建立另一張 PR。所有路徑最終都收斂到
`converge-release-tag` 這同一行呼叫——不是兩套各自維護、恰好長得很像的邏輯，而是結構上
只有一份實作，呼應 #589 決定本身的第一原則（單一 repo-local 腳本被兩種呼叫方式共用）。
`.csarc/scripts/publish-release` 之後唯二對同一 Release 的寫入是 `gh release edit "$tag"
...
3. 真正跨版本持續有效、不是「這一版特有」的已知限制（例如上一節的 hosted
   ...
   狀況脫節的內容——這類內容留在 `.csarc/docs/ci-policy.md` 與 repo-site，Release 說明文字不必
...
1. 上面盤點已經證明 hosted 與本機路徑呼叫的是同一支 `.csarc/scripts/publish-release`（進而
   呼叫同一支 `.csarc/scripts/converge-release-tag`）——不存在兩份平行實作會長期漂移的風險；
...
3. 真正值得防的風險——未來有人繞過 `converge-release-tag`、另開一個呼叫路徑，各自帶
   ...
   `tests/test_release_notes_format.py` 落地這個斷言，掛在 `.csarc/scripts/verify-fast`
   既有的 `uv run pytest` 範圍內，每次 PR 都跑，不需要另外的 full-tier 專屬階段。

**下發到 `template/`：** `.csarc/scripts/converge-release-tag`／`.csarc/scripts/publish-release` 已
透過 `.csarc/scripts/sync-paired-files.sh` 與 root 逐位元組同步（見上兩節），本節的格式契約
...
這筆文字能補足本機執行缺少 hosted log 的公開稽核脈絡，但 commit 訊息與 Issue／PR 留言都是可變、可重播的聲明，無法證明 GitHub 上的 Release 狀態。`.csarc/scripts/check-release-drift` 仍會讀取、驗證格式並在摘要與追蹤 Issue 顯示最新一筆作為診斷資訊，但不讓它改變 drift 結果；讀取稽核資料若失敗只會留下 warning 並當作無紀錄，不得阻斷權威判定與告警。有效紀錄只接受 `operator`、`commit`、`command`、`result` 四欄，其中 `command` 最長 512 字元且不得含反引號，以免稽核文字撐爆 Issue body 或跳脫行內 code。只有 GitHub API 回報的 immutable stable Release，或 job steps 證明明確判定 `no-release` 的成功 `release.yml` run，能抑制告警。

**這支 workflow 只偵測與通知，不接手發版**：既不會自動觸發 `release.yml` 重跑，也不會自動執行 `.csarc/scripts/publish-release`；是否接手仍由人或 agent 判斷，維持 #589 既有的「人或 agent 主動決定啟用」設計原則。
偵測到 drift 且成功建立或更新追蹤 Issue 後，workflow 以 success 結束；只有偵測器、GitHub
API 或通知寫入本身失敗才回報 failure。Drift 事實由追蹤 Issue 承接，不以故意失敗的排程
run 重複表達。

**下發到 `template/`**：`.github/workflows/release-drift.yml` 與 `.csarc/scripts/check-release-drift` 透過 `.csarc/scripts/sync-paired-files.sh` 與 root 保持逐位元組同步，並在 `copier.yml` 重用既有 `.github/workflows/release.yml` 的 `project_mode == 'new'` exclude 條件，不新增第二個 Copier 選項——生成 repo 只要擁有 `release.yml`（`release_ownership == csarc-owned`）就會同時擁有這支漂移檢查，兩者不會分開存在。
