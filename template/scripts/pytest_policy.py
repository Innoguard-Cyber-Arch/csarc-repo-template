...
REQUIRED_FIELDS = {"owner", "issue", "reason", "expires", "remove_when"}
    ...
    *,
    ...
    if args or set(values) != REQUIRED_FIELDS:
        raise pytest.UsageError(
            "quarantine requires owner, issue, reason, expires, and remove_when"
        )
    if (
    ...
    if not isinstance(values["reason"], str) or not values["reason"].strip():
        raise pytest.UsageError("quarantine reason must be non-empty")
    try:
