...
def test_pr_policy_runs_current_main_gate_only_for_final_promotion() -> None:
    """Ordinary Milestone work must not run the final-promotion gate."""
    root = Path(__file__).parents[1]
    workflow = (root / ".github/workflows/pr-policy.yml").read_text()

    step = workflow.split(
        "- name: Require final promotion to contain current main", 1
    )[1].split("- name:", 1)[0]
    assert "github.event.pull_request.base.ref == 'main'" in step
    assert "python3 scripts/delivery_sync.py gate" in step
