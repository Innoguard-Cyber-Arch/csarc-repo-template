...
gh auth status`
    ...
    // copy outcome. Relying on the clicked button's own text change is not
    // enough -- some screen readers only announce a focused control's
    // label change on the *next* interaction, not the moment it happens --
    // so this region is the actual, reliable feedback channel; the button
    // text swap stays as the sighted-user confirmation.
    ...
    function announceCopyStatus(message) {
      ...
      // Force a DOM mutation even when the same message repeats back to
      // back (e.g. two quick failed copies) so assistive tech re-announces.
    ...
    // Issue #681/#682: the install-command overlay (and the term-trigger
    // glossary popovers sharing the same `.config-overlay` markup) is a
    // true modal dialog now, not a passive `role="region"` aside -- it
    // takes focus on open, traps Tab inside itself, and returns focus to
    // whichever button opened it on close (Esc, the close button, or a
    // click outside the card all go through this one function).
    ...
    function openConfigOverlay(overlay, trigger) {
    ...
    addEventListener('keydown', event => {
      const overlay = document.querySelector('.config-overlay:not([hidden])');
      if (!overlay) return;
      if (event.key === 'Escape') {
    ...
    setupOverlay.tabIndex = -1;
    setupOverlay.setAttribute('role', 'dialog');
    ...
    capabilitySlide.querySelectorAll('.setup-trigger').forEach(trigger => {
      ...
      trigger.addEventListener('click', () => {
        ...
        openConfigOverlay(setupOverlay, trigger);
      });
    });
    ...
    document.querySelectorAll('.term-trigger').forEach(trigger => {
      ...
      trigger.addEventListener('click', () => {
        ...
        closeConfigOverlays();
        if (isOpen) return;
        openConfigOverlay(overlay, trigger);
      });
