...
This repo's governance rules live in three places, each with a distinct role: `AGENTS.md` (the rules an agent and a person both follow), `.csarc/policies/*.json` (the GitHub settings this repo expects), and `.csarc/config.yml` (this project's actual choices):
...
- **`.csarc/policies/*.json`:** expected settings (branch protection, required checks, labels) applied and verified by `.csarc/scripts/apply-repository-settings.sh`.
