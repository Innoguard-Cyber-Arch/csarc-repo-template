...
| Concurrency／同步：新 main 不得沿用舊 ancestry 結果 | `tests/test_delivery_sync.py::test_gate_rejects_a_stale_stacked_head`、`test_second_main_advance_invalidates_previous_success` | delivery-sync check 與受審查的 `sync/main-to-*` PR | 保留既有 stale-head guard；真正同時發生的事件競爭與 Ruleset enforcement 仍是平台限定缺口，#240 尚未整合的控制不列為既有覆蓋 |
