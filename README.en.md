...
Available today: a common CI/CD baseline plus independently selectable Python, Rust, and TypeScript language modules, along with Issue/spec, PR checks, and verification. `features` can select `repo-site` and `docker`: new projects default to the portable bilingual site only, while Docker defaults off; an unselected capability creates no files, workflows, or checks. Automated version PRs, GitHub Releases, packaging, checksums, and SBOMs remain candidates until proven by a default-branch live run. Registry publishing and general deployment remain disabled. The settings script reports the account plan separately from live API capability.
...
Selecting `docker` in `features` (Issue #554) creates the Dockerfile, compose starter, and read-only, non-pushing build-and-scan workflow. Selecting `repo-site` creates the bilingual sources, portable HTML, render checks, and Pages desired policy. When Pages is unavailable, the site remains usable offline and the capability is reported as `DEGRADED`, never as deployed.
