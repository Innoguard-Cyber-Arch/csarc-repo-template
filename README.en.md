...
Cyber-Arch's updatable repository foundation: creating a new project, adopting an existing one, and receiving policy updates all preview and verify before a PR merges them. Use the common workflow alone, or opt into Python, Rust, TypeScript, and Go independently.

| Item | Current status |
| --- | --- |
| Template version | v0.29.0-beta.1<!-- x-release-please-version --> |
| Supported languages | Python, Rust, TypeScript, Go (independently multi-selectable; choosing none uses only the common workflow) |
...
| --- | --- |
| Language | Python, Rust, TypeScript, Go are independently multi-selectable; choosing none uses only the common workflow |
...
| Go 1.27.1 | Only with the `go` language module; the generated verifier requires exactly go1.27.1 | Install the `go1.27.1` macOS `.pkg` from the [official Go downloads page](https://go.dev/dl/); do not use `brew install go`, which installs the newest Go rather than the pinned version | Inside WSL2, use the "Linux/WSL2" column | Per the [official Go install guide](https://go.dev/doc/install): `curl -LO https://go.dev/dl/go1.27.1.linux-amd64.tar.gz && sudo rm -rf /usr/local/go && sudo tar -C /usr/local -xzf go1.27.1.linux-amd64.tar.gz`, then add `/usr/local/go/bin` to `PATH` |

On Windows, run the repo itself and the `csarc` CLI from inside WSL2 (Ubuntu); the table's "Windows (native, winget/Chocolatey)" column is for installing an individual tool natively on Windows (e.g. installing `git`/`gh` before entering WSL2) -- the rustup installed there is not on `PATH` once you are inside the WSL2 Ubuntu shell, so use the "Linux/WSL2 (Ubuntu, apt)" column instead. Go is installed only inside WSL2; there is no native Windows path. Inside WSL2, install `gh` from GitHub's official apt repository and confirm `gh --version` is at least 2.93.0; Ubuntu's bundled package is too old for the required Release verification. Choosing Rust also needs `build-essential` (a system C linker) on Linux/WSL2 -- true even for a pure-Rust project with no C bindings; without it, `cargo test` fails at compile time with `error: linker 'cc' not found`. The full guided script for macOS/WSL2 Ubuntu lives on the [repo-site appendix](docs/index.html).

### Developing/contributing to `csarc-repo-template` itself

Beyond `uv` and `gh` from the table above, you also need:

- **pnpm 11, rustup/Cargo, Go 1.27.1**: running the full `./scripts/verify-template.sh` generates and verifies each of the Python, TypeScript, Rust, and Go language modules' own native validator in turn (see `tests/test_language_profiles.py`), and the representative combination test needs all four toolchains. Running just the daily PR gate `./scripts/verify-fast` usually does not need rustup/Cargo or Go, unless a change triggers the template smoke test. Install commands are the same as the table above.
...
- Not choosing `go`: no `go.mod`, `cmd/`, or `internal/` is generated, and no Go toolchain is needed.
- Choosing none of the four (i.e. `ci`): a generated project's `./scripts/verify` only runs the common checks (secret scanning, dependency checks, workflow lint, policy JSON validation, spec validation, etc.), running existing Python tool scripts entirely with `uv run --no-project python`; no Python/Node/Rust/Go project package toolchain is needed, though `uv` itself is still required since these check scripts are written in Python.

This is a separate question from "what tools does developing this template repo itself need": an adopter installing and using a csarc-generated project can choose just `ci`, with minimal prerequisites (see the table above); but contributing to this template repo needs all four language toolchains, because `tests/test_language_profiles.py` generates a project for each of the python/typescript/rust/go profiles and runs its native validator, so the full `./scripts/verify-template.sh` still needs all four available.
...
| PyPI / npm / GHCR / Go module proxy | Not applicable | Root publishes to no registry; a generated project only publishes GitHub Release artifacts and requires no long-lived token; Go is not published to a module proxy and ships no prebuilt binaries |
...
Real, repeatable adoption steps, acceptance evidence, and known platform limitations are collected in [`docs/pilot-adoption.md`](docs/pilot-adoption.md). The first consuming repo, `ai-guardrail`, has completed a v0.2.4 adoption and a v0.3.1 update, proving the shared adopt, update, and live CI paths; Python, Rust, and TypeScript each reached beta through a reproducible create, existing-repo adoption, update, and native-toolchain verification; Go reached beta through the same reproducible create, adoption, update, native, and hosted verification evidence, without a dedicated consuming repo yet. Selecting multiple modules at once does not form a separate profile.
...
`adopt` defaults to a dry run; writing `--dry-run` explicitly is still compatible. It only produces a plain-Markdown adoption report (no longer a PDF) and a machine-readable plan outside the repo, without modifying it or running a target-owned helper or product hook. The adoption report itself carries an independent version number (currently `1.1.0`, i.e. `ADOPTION_REPORT_TEMPLATE_VERSION`, recorded in the report file) and specifically lists added, edited, and removed file counts, impact analysis, a read-only inventory of existing GitHub Issue Types and labels with suggested mappings, and items that need a user decision. The default model uses `Bug`/`Feature`/`Task` for Issue work types; lowercase `bug`/`enhancement`/`documentation` remain PR classifications and the fallback when native Issue Types are unavailable, while documentation keeps the `documentation` label alongside `Task`. Safe aliases can be accepted together with `work_item_mapping_review=accept-safe`, or `review-individually` records that every suggestion was checked; ambiguous custom labels are always preserved. Adoption never renames or deletes remote labels, and repository settings still require the separate plan/apply/check flow. A trial adoption and the completed adoption update the same report file under the same version-control logic, never producing a second file. If every dirty path is an unstaged tracked modification that the plan explicitly lists as `preserve`, the CLI builds and verifies the candidate using the original bytes, allowing the same plan to be applied; any other dirty state can only be reviewed. The plan locks the target HEAD, full working-tree state, Release full SHA, answers, GitHub metadata suggestion snapshot, and output digest; it inventories the metadata again after approval and before the real write, and any drift stops it. Only after `--apply-plan` is approved does the CLI rebuild the candidate in a staged clone, run verification and a patch check, and modify the target repo. README/CHANGELOG stay project-owned, `.gitignore` uses an ordered union, `AGENTS.md` only updates the CSARC-managed block, and a product's existing `release.yml` is kept separate from `csarc-release.yml`.
