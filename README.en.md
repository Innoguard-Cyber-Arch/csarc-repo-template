...
Selecting `docker` in `features` (Issue #554) creates the Dockerfile, compose starter, and read-only, non-pushing build-and-scan workflow. Documentation has its own single switch: `template-and-content` creates portable HTML, render checks, and Pages desired policy; `content-only` manages only project-owned Markdown/README; `off` leaves documentation alone. `i18n` separately controls whether English and Traditional Chinese entry points are maintained. When Pages is unavailable, the site remains usable offline and the capability is reported as `DEGRADED`, never as deployed.
