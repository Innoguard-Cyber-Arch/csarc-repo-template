...
10. Run targeted checks while iterating. Once the minimal solution is explainable and those checks pass, push and open a Draft PR that states scope, completed and pending verification, known risks, and dependencies or non-parallel work. The Issue owner runs the scoped checks selected by the CI plan; ordinary Issue PRs use that scoped evidence for Ready. For final integration, Ready starts the hosted `./scripts/verify-template.sh` gate on the unchanged tree; the integrator runs it locally only when the documented fallback requires local evidence, never in addition to a successful hosted full run. Draft updates need only proportionate targeted checks.
...
12. Drafts use `Refs #N` while work remains. Use `Closes`, `Fixes`, or `Resolves` and mark Ready only after every PR and referenced-Issue item has evidence. Milestone work targets its delivery branch or immediate stack parent; standalone, hotfix, bot, release-please, and promotion work may target `main`. Keep the PR assignee, classification label, and Milestone synchronized from the linked Issue, and request a non-author reviewer outside Draft. Before any automated PR Ready/Draft, authorization, label/milestone, or merge write, acquire its remote lease and use `scripts/pr_lifecycle.py`; other tasks remain read-only. Never merge except under the documented self-merge and quota fallback.
...
- Integrator-owned final delivery check: one hosted `./scripts/verify-template.sh` run on the unchanged candidate tree, or one local run only when the documented fallback applies.
- Issue owners run the CI plan's narrowest scoped checks; those results complement, rather than duplicate, the integrator's final delivery check.
