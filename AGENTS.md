...
- Compose each decision-site page from the existing declarative block syntax in `site/content/*.md`, not new per-page CSS or HTML: it is parsed by `scripts/decision_site_blocks.py` and rendered by named functions in `scripts/build_decision_site.py` (e.g. `render_slide`, `render_detail`, `render_config_guidance`), not Hugo shortcodes or `site/layouts/` templates, which no longer exist. `{{< slide key=… track=… >}}` marks one page section; `{{< basic >}}` holds the default plain-language content most slides need; `{{< legacy >}}` is only for a slide that keeps bespoke fidelity/presentation styling (`legacy="true"` on the slide); simple structural content skips both wrappers and lets the slide body markdownify directly (see the `files` slide). Nest optional deeper technical content inside `{{< basic >}}` with `{{< detail >}}` (always-visible aside) or `{{< disclosure >}}` (collapsed `<details>`); render config examples with `{{< config-guidance track=… >}}` against `site/data/config_examples.json` instead of hand-writing them. Pick the existing block that fits rather than inventing a one-off layout. Mirror `site/content/_index.zh-tw.md` and `_index.en.md` slide-by-slide (same `key`/`track`, matching structure) and keep new `site/data/navigation.json` labels within the width `scripts/check-decision-site-navigation` checks. `site/data/glossary.toml` only feeds the bilingual `llms.txt` index, not an in-page glossary widget.
