...
4. Use the exact checked-in body shape whether creating through the UI, CLI, or API: Issues use `問題`, `完成條件`, and optional `補充`; their kind belongs in native Type and label metadata, not a duplicate body field. Pull requests use `Purpose`, `完成清單`, and optional `補充`. Fold verification, risk, rollback, impact, and decision details into those sections instead of adding parallel top-level sections.
...
12. Use `Closes`, `Fixes`, or `Resolves` only after every PR and referenced-Issue item has evidence. Target the delivery branch or immediate stack parent; only promotion, release-please, qualified hotfix, or audited release recovery may target `main`. Assign the PR author, keep its classification label and Milestone synchronized from the linked Issue, and request a non-author reviewer when it leaves draft. Before any automated PR Ready/Draft, authorization, label/milestone, or merge write, acquire its remote lease and use `scripts/pr_lifecycle.py`; other tasks remain read-only. Never merge except under the documented self-merge and quota fallback.
