...
5. Open or select one GitHub Issue before editing and use its acceptance criteria as scope. When writing or refining acceptance criteria, state which verification is expected to run locally during implementation and which is left for hosted CI to prove; default to hosted CI unless the work itself is a full-tier delivery boundary, per `docs/ci-policy.md`'s verification-tiering principle. Titles use 12-80 ASCII characters, at least three words, no type prefix, and no trailing period. Set one native Type, matching label, assignee, and applicable parent and Milestone at creation time; `scripts/gh-issue-create` nudges (never blocks) when one is likely missing. Once a Milestone closes, `gh issue edit --milestone <name>` can no longer find it by name -- see `docs/ci-policy.md`'s Milestone safeguard section for the `gh api ... --method PATCH -f milestone=<number>` fallback. The creator is the default owner: UI creators assign themselves, while CLI and agents pass `--assignee @me`; change the assignee only for an explicit handoff.
