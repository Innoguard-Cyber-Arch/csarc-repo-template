...
7. Start from the Issue's delivery branch (`dev/m<milestone-number>-<slug>` or standalone `dev/next`) or a work branch whose PR chain ends there. Use `gh issue develop <issue> --base <delivery> --name type/<issue-number>-short-slug` for the native Development link. Conflicting squash-sync promotion uses `promote/m<milestone-number>-<slug>` per `docs/ci-policy.md`. A standalone Issue needing its own soak may use `dev/i<issue-number>-<slug>` with no Milestone and a `promotion` label; only a standalone `fix/*` plus `hotfix` may otherwise target `main`.
...
12. Before using `Closes`, `Fixes`, or `Resolves`, complete every task in the pull request and referenced Issue; do not check work that lacks its stated evidence. Open an Issue pull request against its delivery branch or immediate parent in the stack; only a promotion, release-please, or qualified hotfix may target `main`. Keep the PR assignee, classification label, and Milestone synchronized from the linked Issue, and request a non-author reviewer when it leaves draft. Never merge it yourself except under an explicit repository self-merge policy and the quota fallback documented in `docs/ci-policy.md`.
