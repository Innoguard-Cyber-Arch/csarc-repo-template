...
- [Journey 08](docs/index.html#governance) and the [`docs/ci-policy.md` quota fallback](docs/ci-policy.md#actions-額度-fallback) own review requirements, merge eligibility, Alpha self-merge, and quota fallback. Do not restate or invent exceptions here.
...
7. Start from the Issue's delivery branch, or an open work branch whose pull request chain ends there. One active Milestone owns exactly one `dev/m<milestone>-short-slug` delivery branch, and all of its Issue branches end there. Use `gh issue develop <issue> --base <delivery> --name type/<issue-number>-short-slug` for the native Development link. Follow `docs/ci-policy.md` for delivery, synchronization, promotion, hotfix, and release-recovery routes.
