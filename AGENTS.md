...
7. Use short-lived `dev/m<milestone-number>-<slug>` for Milestones; other work uses `main`. Name Issue branches `type/<issue-number>-short-slug` and stack toward their integration branch. Ordinary PRs need only delivery. Sync `main` at final promotion; early sync needs an owner-recorded dependency. Use matching `promote/m<milestone-number>-<slug>` for conflicts; never rewrite delivery. `main` is permanent.
...
12. Drafts use `Refs #N` while work remains. Use `Closes`, `Fixes`, or `Resolves` and mark Ready only after every PR and referenced-Issue item has evidence. Milestone work targets its delivery branch or immediate stack parent; standalone, hotfix, bot, release-please, and promotion work may target `main`. Before any automated PR Ready/Draft, authorization, label/milestone, or merge write, acquire its remote lease and use `scripts/pr_lifecycle.py`; other tasks remain read-only. Never merge except under the documented self-merge and quota fallback.
