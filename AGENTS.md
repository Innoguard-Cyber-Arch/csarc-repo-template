...
- Reusable validation lives in `scripts/` and tests. `./scripts/verify-fast` (docs or fast tier) and `./scripts/verify-template.sh` (full delivery verification, reserved for Milestone/canary delivery, hotfix, merge queue, manual dispatch, and unknown-risk paths) run locally, not on the hosted runner: the `verify` required check no longer re-executes either script, it only validates the `Verified-locally:` trailer each one writes onto the current commit on success (Issue #661) — exact tree-hash match, freshness, and tier sufficiency, via `scripts/check-verify-attestation`. Running the tier-appropriate script locally before every push is therefore mandatory, not optional: skipping it leaves the hosted check nothing to validate, and it fails closed. Do not run `verify-template.sh` locally for an ordinary fast- or docs-tier PR; see working loop step 11 for the full-tier once-per-PR rule, including why its base-only re-merge exception now needs a fresh local run too. Checked-in files under `.github/workflows/` are the only active GitHub Actions. See [CI/CD settings](docs/index.html#testing) and [`docs/ci-policy.md`](docs/ci-policy.md) for the tier boundary and stage ownership.
...
11. Delegates run scoped or focused checks while iterating. CI selects docs, fast, or full automatically by changed paths and delivery stage (`docs/ci-policy.md`); an ordinary fast- or docs-tier PR needs no local full run, but still needs a local `./scripts/verify-fast` run before every push (Issue #661 — see the Responsibility map bullet above) so the hosted `verify` check has a fresh attestation to validate. Only when the PR itself is a full-tier delivery boundary (Milestone/canary delivery, hotfix, merge queue, manual dispatch, or an unknown-risk path) does the pull request owner or integrator run `./scripts/verify-template.sh` once, locally, on the final candidate tree. This "once" is literal, not merely the first time: after that genuine local full green run, a later required re-merge of the same PR that only brings in further upstream base movement — clean merge, zero file overlap with this branch's own already-verified changes, and no touch to verification/CI/policy infrastructure — does not need another *full* local run; see `docs/ci-policy.md`'s "Base-only re-merge 例外（#468）" section for the precise condition, what does not qualify, and `scripts/check-base-only-remerge` for a checkable version of it. That section's final step, "push and trust the hosted `verify` check", no longer holds as written: #661 removed the hosted re-execution it relied on, so a base-only re-merge's new tip commit has no attestation of its own and the hosted check now fails closed on it exactly like any other unattested push. Until a follow-up Issue reconciles the two mechanisms, treat "skip the local rerun" as not currently available for a full-tier PR even when all four re-merge conditions hold — run `./scripts/verify-template.sh` again locally, the same as a first-time full-tier boundary.
...
- Daily PR gate: `./scripts/verify-fast`, now mandatory before every push (Issue #661) — on success it writes a `Verified-locally:` trailer onto HEAD, which the hosted `verify` check validates instead of re-running anything.
...
- Self-check an attestation before pushing: `./scripts/check-verify-attestation HEAD` (add `--required-tier full` to match what a full-tier PR needs).
