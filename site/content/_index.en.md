...
| Project choice | `release_levels_enabled`, `default_release_level`, `release_level_*_{review,verification}` | this template root disables work-item tiers, defaults to alpha, and maps every level to self/baseline; new repositories enable alpha by default, while adoption enables beta by default; each level maps to self/peer and baseline/fast/docs/full | Issue/Milestone resolution, the `review` gate, verification attestations, and release notes |
