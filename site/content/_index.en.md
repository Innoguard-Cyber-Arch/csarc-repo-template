...
The `Innoguard-Cyber-Arch` API reports this repository is on the Free plan with **public** visibility. GitHub already has an `enforcement: active` Ruleset named "CSARC protected branches" (created 2026-09-03), applied to `main` and `dev/m*`: all three `title`/`verify`/`review` status checks must pass, the review check evaluates `solo`/`peer` and available Copilot evidence, and force-push is forbidden. This repository explicitly enables `actions_fallback: admin`, but the lifecycle may use the admin bypass only after proving a zero-step billing block and revalidating all other evidence.
