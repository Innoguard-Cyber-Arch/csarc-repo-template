...
The `Innoguard-Cyber-Arch` API reports this repository is on the Free plan with **public** visibility. GitHub already has an `enforcement: active` Ruleset named "CSARC protected branches" (created 2026-09-03), applied to `main` and `dev/m*`: it requires at least 1 approval, CODEOWNER review, and all three of the `title`/`verify`/`review` status checks to pass, and disallows force-push -- `main` genuinely does have enforced merge protection today.
