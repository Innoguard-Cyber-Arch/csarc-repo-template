...
{{< legacy >}}
      <header>
        ...
        <p class="subtitle">共用規則與 9 條現行 GitHub Actions 每個新 repo 都有；治理漂移排程依 <code>enable_governance_drift_check</code> 產生，語言工具與產品目錄則依未選語言、Python、Rust、TypeScript 或多語言模式產生。</p>
      </header>
      <div class="repo-map-window" aria-label="模板產生檔案的視覺對照">
        ...
        <div class="repo-tree-body">
          ...
          <div class="repo-tree-row"><span class="repo-tree-path depth-1 folder">workflows/</span><span class="repo-tree-purpose"><span class="journey-code">01／03／04／05／06／07／08</span><span class="purpose-copy">9 條共用流程＋1 條選配排程</span></span><span class="scope-badge shared">共用</span><span class="owner-badge template">公版主導</span></div>
          ...
          <div class="repo-tree-row"><span class="repo-tree-path depth-2">release.yml</span><span class="repo-tree-purpose"><span class="journey-code">07</span><span class="purpose-copy">版本 PR 與 verified GitHub Release</span></span><span class="scope-badge shared">共用</span><span class="owner-badge template">公版主導</span></div>
...
| `.github/workflows/` | 9 條現行流程：工作單整理、里程碑同步、規格開單、PR 規則、必要驗證、漏洞排程、reviewer 指派、工作關單與發版，加上選配的治理漂移排程 | 公版主導 |
...
<aside class="selection-note"><strong>Current state｜2026-09-01</strong><span>一支 release workflow 負責版本 PR 與 verified immutable GitHub Release；registry、attestation 與部署仍未自動啟用。</span></aside>
{{< legacy >}}
      <header>
...
| 正式版本與 CHANGELOG | Active | Release Please 建立同一張受審查 PR；已發布版本不改寫 |
| tag／GitHub Release | Active | 版本 PR 合併、完整驗證與成品重驗後自動發布 |
| checksum／SBOM | Active | 與 exact-tag 成品一起建立，下載後重新驗證 |
...
{{< legacy >}}
      ...
      <div class="plan-grid">
        <article class="plan-card current"><h3>Free <span class="plan-state">目前</span></h3><p><strong>保留審查設定，強制能力降級：</strong><code>.github/REVIEWERS</code> 保存 reviewer 名單；private repo 只把期望 Ruleset 保留在 <code>policies/rulesets.json</code>，因為 REST 與 GraphQL 建立 API 都會拒絕。check 標示 DEGRADED。</p><ul><li>Action 會排除 PR 作者並輪派一位個別 reviewer</li><li>這不是 team request 或 merge gate，審查仍需由人完成</li></ul></article>
      ...
      <aside class="selection-note"><strong>部署原則</strong><span><code>plan</code> 先查帳號方案、repo 可見性、repository teams 與 Ruleset API；team 不存在、不可見或沒有 repo write access 時直接停止，不能再被 Free private 的降級路徑掩蓋。導入時可設定一到多位 reviewers；Action 只從 base branch 讀取名單，排除作者後提出一位個別 review request。Free private 不支援 team request，也無法強制核准，因此這項自動化不等於 merge gate。Web UI 可供管理員人工預建 disabled Ruleset，但公開 REST／GraphQL API 無法自動建立。腳本本身不會改可見性。依序執行 <code>plan</code>、<code>apply</code>、<code>check</code>；可修正差異 fail-closed，不支援的 Ruleset 或受限制的管理欄位回報具體 DEGRADED。完整管理欄位驗證應由管理員在可信任 checkout 使用 Administration read 憑證，不把該 token 暴露給 PR 程式碼。GitHub 方案升級、不可逆操作與組織權限變更都需組織 owner 另案核准；模板不會自行執行。</span></aside>
      <aside class="config-guidance"><strong>設定方式</strong><ul><li><strong>查方案與 API：</strong><code>scripts/apply-repository-settings.sh plan</code></li><li><strong>套用支援的設定：</strong><code>scripts/apply-repository-settings.sh apply</code></li><li><strong>比對全部可觀察設定：</strong><code>scripts/apply-repository-settings.sh check</code> 比對 CODEOWNERS、repository、Actions、政策標籤與有效 Ruleset</li><li><strong>指定 reviewer：</strong><code>reviewers</code> 產生 <code>.github/REVIEWERS</code>；Action 呼叫 <code>scripts/request-reviewer</code></li><li><strong>檢查治理漂移：</strong><code>enable_governance_drift_check</code> 開啟時，生成 repo 每日與手動呼叫 <code>scripts/check-governance-drift</code>；本模板 source repo 不啟用排程</li></ul></aside>
      <table class="decision-register" aria-label="GitHub 方案與 apply／check 行為對照">
        ...
        <tbody>
          ...
          <tr><td>Free organization＋private</td><td>套用基本設定，並把期望 Ruleset 保留在 <code>policies/rulesets.json</code>；公開 API 無法建立 Ruleset</td><td>標示 <code>DEGRADED</code>，Action 提出一位個別 review request；team request、紅燈或未核准都不能成為 merge gate</td></tr>
...
| Free organization＋private | 套基本設定，期望 Ruleset 留在 `policies/rulesets.json` | 標示 `DEGRADED`，自動請一位同事 review；無 merge gate |
...
依序執行 `scripts/apply-repository-settings.sh plan`、`apply`、`check`。`check` 比對 CODEOWNERS、repository、Actions、政策 labels 與有效 Ruleset。所有 repo 都從 base branch 的 `.github/REVIEWERS` 排除作者後請一位同事 review；這不是 merge gate。`enable_governance_drift_check` 開啟時，生成 repo 每日重跑本機可用的 `scripts/check-governance-drift`，可修正偏離會開立或更新一張追蹤 Issue；關閉時不產生兩個檔案。本模板 source repo 只保留程式供下發與本機驗證，不另跑每日排程。
...
{{< legacy >}}
      ...
      <table class="bridge-table" aria-label="五月版簡報與目前設計的逐頁對照">
        ...
        <tbody>
          ...
          <tr><td>p.6</td><td>PR 與審查</td><td><span class="bridge-status adjust">強化</span></td><td><details class="bridge-detail drop-down"><summary>Issue、編號分支與 PR 形成固定鏈</summary><div class="bridge-popover"><p><strong>五月版｜</strong>PR 是保護分支的唯一入口，方向保留；三層審查改成依風險增加審查者。</p><p><strong>本次判斷｜</strong>一般 PR 要有同編號 Issue、CI 與一位同事；高風險架構變更另附決策紀錄。</p><p><strong>落地方式｜</strong>分支固定 <code>type/123-short-slug</code>，PR 內文固定 <code>Closes #123</code>；Action 會輪派一位非作者 reviewer，但 Free private 仍無法把核准強制成 merge gate。</p></div></details></td></tr>
...
<aside class="selection-note"><strong>Current state｜2026-09-01</strong><span>下方 technical view 的 promotion／兩段式 release handoff 是歷史設計。現行 CI 依風險分級；Release Please 版本 PR 與單一 release workflow 分別負責審查與發布。</span></aside>
{{< legacy >}}
      <header>
        <span class="selection-sequence">決策附錄</span>
...
<aside class="selection-note"><strong>Current state｜2026-09-01</strong><span>下方 technical view 保留舊 run 與當時判斷供稽核；Release Please 已由單一 release workflow 採用，release consumption 與 live-integration 專用 workflows 仍未恢復。</span></aside>
{{< legacy >}}
      <header>
        <span class="selection-sequence">決策附錄</span>
