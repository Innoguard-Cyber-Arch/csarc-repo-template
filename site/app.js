...
scripts/apply-repository-settings.sh`
        },
        {
          ...
          summary: '測試會真的執行 CLI init、adopt dry-run／machine plan／隔離驗證／apply、update check／dry-run／apply 與衝突 fail-closed；adopt dry-run 不修改 target repo，正式套用拒絕任何 HEAD、working tree、release、answers 或輸出漂移。CLI 內部仍以 Copier 產生與 smart update。root-only 測試資產若混入生成 repo 也會失敗。',
...
uvx --python 3.14 --from csarc-repo-cli csarc init ./my-project \\
  --yes --non-interactive`
      ...
      existing: {
        title: '把公版導入既有 repo',
        goal: 'CLI 自行找出 Git root；--dry-run 可讀取 dirty tree，但只在 repo 外產生 Markdown、PDF 與不可套用的 machine plan。乾淨 tree 會先在暫存 clone 完成固定合併、產品 hook 與完整驗證，再鎖定可套用 plan。',
