...
GitHub 只會從固定位置讀取部分檔案，例如 `.github/workflows/`、`.github/SECURITY.md` 與 root `AGENTS.md`；語言工具也可能要求 root manifest。這些平台或產品檔案不能為了視覺整齊而搬走。
...
- 平台固定檔保留在 `.github/`；root `AGENTS.md` 只作自動探索入口，詳細規則放 `.csarc/docs/agent-workflow.md`，`.claude/CLAUDE.md` 只引用 root 入口。
...
| --- | --- | --- | --- |
| 最小新專案 | README、CHANGELOG、AGENTS、`.gitignore`、`docs/`、空的 `src/` 與忽略的 `dist/` 建置目錄 | `.csarc/`、`.github/`、`.claude/` | 不產生 root `scripts/`、`policies/` 或 `site/` |
