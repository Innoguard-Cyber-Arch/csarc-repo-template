...
- PR lifecycle 寫入以 repository remote ref 作 atomic single-writer lease；default-branch PR 同時取得 promotion lane lease。Lease 綁定 task owner、GitHub actor、PR、base、完整 head SHA 與期限，過期或無法驗證時停止，不自動搶鎖。Agent 的 Ready／Draft、label／milestone、authorization preparation 與 merge 一律走 `scripts/pr_lifecycle.py`，其他 task 維持唯讀。
