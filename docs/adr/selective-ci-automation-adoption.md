...
| 已退役 | 容器 profile 的 `none`、`verify`、`ghcr` 三種模式 | 盤點後沒有 active build／scan／publish workflow；#430 移除無執行者的 Copier 選項。產品有真實需求時另開 Issue。 |
...
原容器設計的 Buildx、smoke、Trivy、GHCR 與 attestation 路徑只保留在 Git／Issue 歷史，不能當成現行能力。模板目前不生成容器 job、Docker Dependabot 或 registry 權限。
...
回退官方 Actions 分組只需移除 `groups.official-actions`，個別更新仍會繼續提出。既有產品的 Containerfile、workflow 與部署仍由產品擁有，Copier 不因移除模板選項而接管或覆寫。
