...
Promotion fallback 只透過 `promotion_gate.py finalize-quota-fallback` 產生 `release_eligible: false` 的 evidence；merge 後由 `promotion_gate.py verify-quota-main` 核對 main identity。
