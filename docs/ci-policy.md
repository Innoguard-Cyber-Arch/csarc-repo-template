...
| Version／Release | `.github/workflows/release.yml` | #369／#430／#588／#591 | `main` push（post-merge）、manual rerun | top-level read；單一 release job 才有 `contents`／PR／Issue／status write；30 分鐘 | Automatic 或 Guided 版本 PR；合併後由同一 workflow 發布 tag／GitHub Release／成品／checksum／SBOM | `tests/test_release_policy.py`、`tests/test_release_bundle.py`、`tests/test_journey07_release.py` | 2026-09-03 重新查證，取代先前「尚未落地」的過時敘述：`gh api actions/workflows` 顯示本檔已在 `main` 註冊為 `active`，且 `gh api tags`／`releases` 顯示過去確有真實 live 發版（`v0.12.2`／`v0.12.1`／`v0.12.0`／`v0.11.0`／`v0.10.1` 等，最新 `v0.12.2` 發布於 2026-08-27）。但 `main` 目前連續紅燈，已定位三個各自獨立的成因：(1) run [33744438607](https://github.com/Innoguard-Cyber-Arch/csarc-repo-template/actions/runs/33744438607)（commit `1177157`，2026-09-03T10:28Z）與 run [33744428560](https://github.com/Innoguard-Cyber-Arch/csarc-repo-template/actions/runs/33744428560)（commit `49fcfe1`／PR #569，2026-09-03T10:27Z）在 `Static assets and paired files` 階段失敗於 `docs/index.html is stale`——根因是 `scripts/verify-fast` 的文件 staleness check 只在 `tier == docs` 觸發、未依 `scopes` 是否含 `docs` 判斷，混合 scope 的 PR（如 #569）因此在 PR 階段完全跳過這項檢查，修正見 #588；(2) run [33719533651](https://github.com/Innoguard-Cyber-Arch/csarc-repo-template/actions/runs/33719533651)（commit `9ed3594`，2026-09-03T05:36Z）與 run [33730000169](https://github.com/Innoguard-Cyber-Arch/csarc-repo-template/actions/runs/33730000169)（commit `99f52ef`，2026-09-03T07:48Z）在其餘四個階段全數 PASSED 後，於 `Regression tests` 階段失敗於 `rm: cannot remove '.../work/.git': Directory not empty`，根因尚未定位，追蹤於 #591；(3) run [33724898939](https://github.com/Innoguard-Cyber-Arch/csarc-repo-template/actions/runs/33724898939)（commit `7719d2e4`，2026-09-03T06:55Z）與 run [33729747815](https://github.com/Innoguard-Cyber-Arch/csarc-repo-template/actions/runs/33729747815)（commit `ed7ab25`，2026-09-03T07:52Z）在 `verify-template.sh` 全數通過後，於發版前的 capability preflight 因 GitHub Actions policy 端點回傳 HTTP 403（`repository Actions policy could not be read`）而以 `BLOCK_REASON: ... immutable_releases` fail-closed——這是 #123 設計的既知 allowed／blocked／unknown 三態模型刻意擋下，不是 bug，但代表在目前這個 repo／org 的 Actions 權限設定下，release 仍無法自動完成 | active（歷史上已真實發版）；但目前 `main` 為 blocked，需先解決 #588 與 #591 兩個真實成因，且 #123 的 fail-closed 權限限制仍未解除，才會有下一次乾淨的 live run |
