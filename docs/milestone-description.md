...
個別 work PR 除了 `scripts/validate-pr-policy` 的結構檢查，還要通過 `review` required
check。`admin_bypass` 允許時可使用 exact-head 自我授權，否則必須是非 PR 作者的
exact-head 核准；後續 push 會使舊核准失效。GitHub Ruleset 永遠保留必要 status checks
且不設 bypass，lifecycle 也會重驗所需 CI 與留下稽核證據。
