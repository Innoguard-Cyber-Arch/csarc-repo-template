...
`docs/index.html` 是可下載、轉寄並以 `file://` 離線開啟的簡報交付物；它不是唯一的可編輯決策來源。需要理解或修改內容時，先依下表找到對應來源，不要直接把新決策只寫進簡報。

| 類型 | 路徑 | 用途與維護方式 |
| --- | --- | --- |
| 單檔交付物 | `docs/index.html` | 由 `site/` 重建的 portable presentation；內嵌樣式、程式與媒體，不直接編輯 |
| AI 閱讀索引 | `llms.txt`、`docs/llms.txt` | 由 Hugo 從 `decision-site/data/glossary.toml` 同源產生；兩份輸出都不直接編輯 |
...
- 公版維護者負責 `decision-site/data/glossary.toml`；調整詞彙後執行 `./scripts/build-hugo-preview`，並提交兩份 `llms.txt` 產物。`csarc-ai-setup` 的 `docs/proposal-deck.html`／`SDD.md` 沒有共用此內容來源，本次不跨 repository 同步；若該 repo 需要相同能力，另開獨立 Issue 評估。
