...
| Concurrency／同步：新 main 不得沿用舊 ancestry 結果 | `tests/test_delivery_sync.py::test_gate_rejects_a_stale_stacked_head`、`test_second_main_advance_invalidates_previous_success` | `title` check 內的同步 gate 與受審查的 `sync/main-to-*` PR | 保留既有 stale-head guard；#240 lease controls 已由 #266 candidate 整合，但真正同時發生的事件競爭與 Ruleset enforcement 仍須 live walkthrough，candidate merge／啟用前不列為 production 覆蓋 |
