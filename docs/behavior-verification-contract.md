...
| --- | --- | --- | --- |
| CI router、verifier、test harness、generator 或 CLI adoption／update 變更不得偽裝 docs-only | `tests/test_ci_tier.py::test_high_risk_families_are_explicit`、`test_issue_and_integrated_stages_apply_risk_differently`、`test_ready_high_risk_work_runs_related_deep_checks`、`test_verifier_changes_are_explicit_risks` | `ci-plan-*` 的 stage、scope、risk、runtime 與 adoption route | Issue branch 跑 scoped fast；generator／CLI 才加 macOS adoption，verifier／test harness／release 加 runtime；直接進 main 的整合風險升 full |
...
- **已消除的重複：**一般 Issue PR 只跑非 `large` 的 scoped fast；final Ready candidate
  ...
  `runtime`，TypeScript 只跑一次，macOS adoption 只由 generator／CLI adoption
  風險與 scheduled deep matrix 啟動；release／security／promotion 不會無條件觸發
  不相關的 macOS E2E，OSV／Zizmor 與 focused policy tests 依風險或週期執行。
