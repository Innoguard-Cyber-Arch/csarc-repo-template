...
真實導入的可重複步驟、驗收證據與已知平台限制整理在 [`docs/pilot-adoption.md`](docs/pilot-adoption.md)。第一個 consuming repo `ai-guardrail` 已完成 v0.2.4 導入與 v0.3.1 更新，因此共用治理與 CI/CD-only 基線為 beta；尚未有真實採用證據的 Python、TypeScript 與 Rust 語言模組維持 alpha。同時選取多個模組不會形成另一種 profile。
