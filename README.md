...
> 若 plan 顯示 `WARNING main is not protected`，部署者必須選一條路：需要保密就請組織 owner 在 **Organization Settings → Billing & licensing** 升級至 GitHub Team 以上；已核准公開的程式碼才可在 **Repository Settings → General → Danger Zone** 改為 public。接著建立或確認 `.github/CODEOWNERS` 指定的 team，重新執行 `plan`，看到 `APPLY policies/rulesets.json` 後才執行 `apply`。不要為了省方案費用擅自公開 repository。
