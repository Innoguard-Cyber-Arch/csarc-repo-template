...
- **線上整合證據：**`./scripts/verify-template.sh` 只證明靜態與合成驗證；root-only `Live integration smoke` 才會在本 repo 實際 dispatch OSV、Release Please、release handoff 與 governance drift，並為每項能力保存 JSON artifact。執行方式、副作用與分層排錯見 [`docs/live-integration.md`](docs/live-integration.md)。
