...
- **PR 自動要求同事審查：**`.github/workflows/governance-comment.yml` 在非 draft PR 開啟、重開或轉為 ready 時，從 base branch 的 `CODEOWNERS` 讀取 team 並送出 review request；workflow 只執行受信任的 base 程式碼，使用短效 `GITHUB_TOKEN`，不需要 PAT 或 GitHub App。Free private 也能提出 review request，但不能強制對方核准或阻止合併；完整 merge gate 仍需支援 Ruleset 與 private CODEOWNERS 的方案。
...
| Free organization＋private | 套用基本設定，並把期望 Ruleset 保留在 `policies/rulesets.json`；公開 API 無法建立 Ruleset | workflow 仍自動要求設定的 team review，並以 `DEGRADED` 警告未強制；紅燈或未核准仍不能阻止直接 push／手動合併 |
...
程式與政策審查者以 `.github/CODEOWNERS` 為準；governance workflow 會自動向該 team 提出 review request。一般缺陷與可重現問題使用 GitHub Issue；疑似資安事件或敏感資料走組織核准的內部通報管道，不貼到 Issue，詳見 [`SECURITY.md`](SECURITY.md)。
