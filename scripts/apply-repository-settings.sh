...
if [[ "$pages_policy_enabled" != "true" && "$pages_policy_enabled" != "false" ]]; then
  echo "Invalid enabled in policies/pages.json: $pages_policy_enabled (expected true or false)." >&2
  exit 1
fi
...
# GitHub Pages reconciliation (Issue #985). `enabled` in policies/pages.json is the
...
PY
  ...
  # toggle (Issue #532): that toggle only skips the policies/repository.json
  ...
  case "$pages_action" in
    noop)
      if [[ "$pages_policy_enabled" == "true" ]]; then
        ...
        echo "Pages opt-out matches policies/pages.json: enabled=false and GitHub Pages is not published for $repo."
      else
        echo "Pages opt-out matches policies/pages.json: enabled=false and $plan_label cannot publish GitHub Pages while $repo is private."
      fi
      ;;
    degraded)
      [[ "${GITHUB_ACTIONS:-}" == "true" ]] &&
    ...
    update)
      ...
      ;;
    disable)
      echo "Pages settings drift: GitHub Pages is published for $repo; policies/pages.json requests enabled=false (explicit opt-out)." >&2
      check_errors=$((check_errors + 1))
      ;;
    *)
...
case "$pages_action" in
  noop)
    if [[ "$pages_policy_enabled" == "true" ]]; then
      echo "- NO-OP policies/pages.json (enabled=true; GitHub Pages already published and matching)"
    else
      echo "- NO-OP policies/pages.json (enabled=false; GitHub Pages is not published)"
    fi
    ;;
  enable) echo "- ENABLE policies/pages.json (enabled=true; create the GitHub Pages site)" ;;
  update) echo "- UPDATE policies/pages.json (enabled=true; $pages_drift)" ;;
  disable) echo "- DISABLE policies/pages.json (enabled=false; unpublish the live GitHub Pages site)" ;;
  degraded) echo "- DEGRADED policies/pages.json: GitHub Pages requires GitHub Enterprise Cloud for a private repository on $plan_label" ;;
  *) echo "- BLOCKED policies/pages.json: cannot inspect live GitHub Pages state; apply fails closed before any change" ;;
esac
if [[ "$policy_actions_permissions" == "true" ]]; then
