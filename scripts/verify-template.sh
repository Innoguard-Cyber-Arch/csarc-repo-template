...
test "$(wc -c < AGENTS.md)" -le 13500
test "$(wc -c < template/AGENTS.md.jinja)" -le 14500
...
for workflow in .github/workflows/promotion.yml \
  template/.github/workflows/promotion.yml; do
...
grep -q 'types: \[opened, reopened, synchronize, ready_for_review, converted_to_draft, labeled, unlabeled\]' \
...
grep -q 'test "$TYPESCRIPT_RESULT" = success' \
  .github/workflows/reusable-ci.yml
grep -Fq 'python3 scripts/delivery_sync.py "${gate_args[@]}"' \
  .github/workflows/pr-policy.yml
grep -q 'python3 scripts/delivery_sync.py merge-group-gate' \
  .github/workflows/pr-policy.yml
grep -q -- '--delivery-base "$PR_DELIVERY_BASE"' \
  .github/workflows/pr-policy.yml
grep -q -- '--head-ref "$PR_HEAD"' .github/workflows/pr-policy.yml
grep -q -- '--pr-number "$PR_NUMBER"' .github/workflows/pr-policy.yml
...
test "$(grep -l 'delivery_sync.py reconcile' .github/workflows/*.yml | wc -l | tr -d ' ')" -eq 1
...
if grep -q 'delivery_sync.py reconcile' \
  .github/workflows/delivery-maintenance.yml; then
  echo "Delivery reconciliation must have one trusted workflow writer."
  exit 1
fi
...
test ! -e "$fixture_root/default-project/scripts/__init__.py"
root_only_paths=(
...
grep -q 'docs/decisions/' \
  "$fixture_root/default-project/docs/README.md"
grep -q 'Never store raw transcripts' \
...
grep -q 'gh auth setup-git' \
  "$fixture_root/default-project/.github/workflows/delivery-sync.yml"
grep -q 'delivery_sync.py merge-group-gate' \
  "$fixture_root/default-project/.github/workflows/pr-policy.yml"
grep -q -- '--head-ref "$PR_HEAD"' \
  "$fixture_root/default-project/.github/workflows/pr-policy.yml"
grep -q -- '--pr-number "$PR_NUMBER"' \
  "$fixture_root/default-project/.github/workflows/pr-policy.yml"
grep -q '"$head_ref" != promote/m\*' \
...
# Squash-merging removed this fixture commit from normal branch ancestry.
if ! git cat-file -e "$legacy_memory_base^{commit}" 2>/dev/null; then
  git fetch --no-tags origin "$legacy_memory_base"
fi
...
test ! -e "$update_project/scripts/__init__.py"
grep -q '^owner = "legacy"$' "$update_project/pyproject.toml"
