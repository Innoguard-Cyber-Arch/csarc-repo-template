...
def promotion_source(base: str, head_ref: str) -> str | None:
    ...
    if base != "main":
        return None
    if DELIVERY_BRANCH.fullmatch(head_ref):
        return head_ref
...
def read_delivery_state(
    ...
    status, payload = api.request(
        "GET", f"repos/{repo}/milestones/{int(match.group(1))}"
    )
    milestone = require_response(status, payload, "read delivery Milestone")
    if (
        not isinstance(milestone, dict)
        or milestone.get("number") != int(match.group(1))
        or milestone.get("state") != "open"
    ):
        raise RuntimeError("Delivery sync requires its matching open Milestone")
